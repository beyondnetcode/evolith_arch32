...
**Last Updated:** 2026-09-20 (counts re-derived from the tree; see the note under §2.1)
...
├── Rulesets (21 categories) and Skills
├── Artifact and Evidence Schemas
├── Phase Gate Definitions
├── OPA Policies (35 .rego aggregated by main.rego) with dual-engine parity
└── Adapter and Integration Contracts
```

> **Where the numbers come from.** Every count in this document (ruleset
> categories, OPA policies, evaluation kinds, controllers, commands, tools, ports,
> adapters) is hand-typed and goes stale; the authority is the tree. The
> generated [Reference Corpus Inventory](../../../reference/core/control-center/maturity-reports/inventory-summary.md)
> and [Product Surface Inventory](../../products/smart-cli/product-inventory.md)
> are regenerated by CI, and `node .harness/scripts/pages/derive-page-metrics.mjs`
> prints the full derived set (kinds, evaluators, controllers, handlers, ports,
> adapters) as JSON. Last re-derived: 2026-09-20.

**ADR-0101: Core is a stateless Evaluation Engine.** Core receives an `EvaluationContext` with opaque identifiers, runs multi-kind evaluations (gate, artifact, evidence, architecture, blueprint, topology, checkpoint, deployment, rule, compliance, design, phase-artifacts), and returns an `EvaluationResult` with non-binding verdicts and recommendations. Core **does not persist** state — Tracker owns the governance state at runtime.
...
2. Core rulesets (21 categories), schemas, standards, taxonomy, and inheritance;
3. the canonical artifact and evidence model;
4. the **EvaluationOrchestrator** with 12 EvaluationKinds and 7 KindEvaluators (the Core API wires 5 of them);
...
8. OPA rulesets (35 policies aggregated by `main.rego`) with dual-engine parity (Native TypeScript + OPA/WASM);
...
flowchart TB
  ...
  subgraph CORE["repo · evolith_arch32 (Evolith Core · Constitution)"]
    ...
    subgraph RT["Agent Runtime · @beyondnet/evolith-agent-runtime"]
      ARS["AgentRuntimeService<br/>11 ports on the hot path<br/>(21 declared · 47 adapter classes)"]
...
|---|---|---|
| **REST API** | Tracker UI, CI/CD, enterprise integrations | 12 controllers: evaluation, gates, phases, architecture, architecture-plan, projects, satellites, capabilities, composable-validate, reference, metrics, health |
| **MCP HTTP/SSE** | LLMs and autonomous agents | 55 tools, 12 resources, 8 prompts: evaluation, validation, agents, ADRs, MoSCoW, drift, configuration |
| **CLI** | Engineers and product roles | 38 commands: validate, evaluate, gate, drift, scaffold, ADR lifecycle, agents, chat, satellite, sdlc |
| **Agent Runtime** | AI agents, chatboxes, external triggers | **11 hexagonal ports on the execution hot path** (7 required, 4 optional), governed orchestration with OPA + HITL. The package *declares* 21 port interfaces (in 20 port files) and 47 adapter classes (in 63 adapter files), including 6 interaction adapters (CLI Command, CLI Chat, Hermes, MCP, OpenCode, External) — see the note below for which of those are wired and which are speculative. |
...
> **A port count is not a capability count.** An inventory number ("21 ports, 47
> adapters") measures how much seam was carved, not how much capability ships.
> Stated explicitly, and re-verified against the code on 2026-09-20:
>
...
> `IApprovalPort`, `ITrackerTracePort`, `IMemoryPort`; and **4 optional** —
> `IAgentEnginePort`, `IKnowledgePort`, `IWorkspaceContextPort`, `IRunJournalPort`.
> Eleven ports for a governed plan → validate → execute → trace pass is well sized,
> and each has at least one adapter that runs.
>
...
CLI · MCP · REST Exposure                 Managed and Certified Adapters
