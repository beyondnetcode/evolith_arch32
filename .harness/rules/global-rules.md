...
| **R-12** | Conventions | Strictly enforce naming prefixes, taxonomies, relative links, and Markdown anchors before merges; approved tool dot-folders include `.claude/`, `.mimocode/`, `.obsidian/`, and `.vscode/`; root-level content directories require accepted ADR authority and `/topologies/` is prohibited unless a superseding ADR changes root taxonomy. |
