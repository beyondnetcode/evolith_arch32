...
**Title:** Three CI checks on PR #275 were red — two were real bugs found by investigating, one is genuine devDependency debt left open on purpose

- **Purpose:** Distinguish, with evidence rather than assumption, which parts of a PR's red CI are pre-existing branch debt versus a regression the PR introduced — fix what a targeted investigation shows is actually fixable, and leave what genuinely is not as a scoped, documented decision rather than force it through.
- **Evidence:** **Three CI checks were red on PR #275 — `Security Audit`, `E2E Tests`, `Evolith Core Validation` — and two turned out to be real bugs the investigation surfaced, not the accepted debt they first looked like.** `Evolith Core Validation` failed at `Bilingual Terminology Lint`: of 5 flagged terms, 2 were genuine untranslated prose (`database` in `0124-credential-secret-management-standard.es.md`, `ports and adapters` in `level-3-components/README.es.md`) and 2 were table/citation cells (`Portability` inside a clause citing ISO/IEC 25010:2023's own English characteristic-rename vocabulary in `gap-tracking.es.md`'s `GT-598` prose; `Modular Monolith` / `Microservices` as 2 of 8 rows in a canonical topology-name table in `maturity-assessment.es.md`) where translating ONLY the flagged term while every sibling stayed in English would have made the document LESS consistent, not more. `E2E Tests` failed on exactly 1 of 132 tests — `should validate architecture with --arch flag` called `validate --architecture --arch-level F1`, and neither flag ever existed in the CLI (`grep -rn "arch-level" src/sdk/cli/src/` returns nothing; `validate --help` names the real flag as `-a, --arch`, a boolean with no level argument). Commander rejected `--architecture` as unknown and exited `1`, which the test's own `[0, 2]` contract cannot distinguish from a genuine crash.
- **Component:** `Cross` · **Criticality:** P2 · **Complexity:** S
- **Provenance:** Found on 2026-07-29 while confirming PR #275's CI was green after the `GT-634` ratchet fix; fixed the same day at the user's explicit follow-up request ("arréglalos los tres" — fix all three), after the initial investigation-only pass.
- **Acceptance criteria:**
  ...
  - [ ] `Security Audit`: the 31 `npm audit` findings are triaged — upgraded where a non-breaking fix exists, or explicitly accepted with a documented reason where it does not.
  - [x] The two fixed checks are re-verified green with the exact CI command, not merely reproduced ad hoc.

**Closure evidence (partial — 2 of 3):** `Evolith Core Validation`: `database`→`base de datos` and `ports and adapters`→`puertos y adaptadores` translated directly in place. The other 3 flagged instances are marked with backticks instead of translated — `` `Portability` ``, `` `Usability` ``→`` `Interaction capability` `` (its untouched neighbor in the same clause), and `` `Modular Monolith` `` / `` `Microservices` `` in the topology table — which `bilingual-terminology-lint.mjs`'s own header comment says it strips as code identifiers before scanning prose; this is a documented "this is a cited term/identifier, not narrative" exception via the mechanism the tool already provides, not a bypass. Re-run: `node .harness/scripts/bilingual-terminology-lint.mjs --verbose` passes; `01-validate-docs` (1491 files) and `04-check-bilingual-parity` unaffected. `E2E Tests`: the flag corrected to `-a, --arch` in `test/e2e/cli-e2e.test.ts`, with a comment recording why `--architecture`/`--arch-level` never worked. Reproducing this locally first required discovering that `npx tsc -b tsconfig.json` (used earlier in this session for unrelated builds) skips npm's `prebuild` lifecycle hook, so `copy-rulesets` never ran and the CLI's bundled `rulesets/` held only a stale `agents/` fixture — every manual repro attempt failed on an unrelated `RULESET_NOT_FOUND` until `npm run build` was used instead, which fired `prebuild` correctly. With the real corpus bundled, `validate --satellite <fixture> --arch` returns exit code `2` (blocked verdict, 61 rules checked, 117 skipped by applicability) — matching the test's own `[0, 2]` contract. Full suite re-verified with the exact CI command: `npm run test:e2e` → 18 suites, 132/132 passed. `Security Audit`: investigated further, not fixed, and found DEEPER than the first pass recorded. Isolated in a scratch git worktree (`git worktree add --detach`) with a REAL, non-symlinked `node_modules` copied from the main checkout — the per-package symlink farm this session built earlier for testing `@beyondnet/*` workspace packages against local sources was confirmed to corrupt `npm ls`'s own dependency-graph display (fabricating edges like "cross-spawn depends on jest"), though NOT `npm audit`'s severity count, which matched 31 in both the corrupted and the clean environment. Traced two real vulnerable chains, both rooted in a `brace-expansion` version the repo's OWN pre-existing `overrides.brace-expansion: "2.1.2"` does not reach: `babel-plugin-istanbul` (pulled in via `ts-jest` → `@jest/transform`, itself via `test-exclude@6.0.0`) and `eslint@9.39.4`'s own bundled `minimatch@3.1.5`, each resolving a NESTED, un-hoistable `brace-expansion@1.1.16` at `node_modules/<parent>/node_modules/brace-expansion` instead of using the root override. Separately, `fast-uri@3.1.2` (under `ajv@8.20.0`) is safely bumpable to `3.1.4` — in-range of `ajv`'s own declared `^3.0.1` dependency, confirmed via `npm view fast-uri versions`. The plain, non-forcing `npm audit fix` cannot even attempt any of this: it fails immediately with an unrelated `ERESOLVE` peer conflict on `@typescript-eslint/parser`. Every one of the 31 flagged packages is a devDependency (the jest/eslint/`@nestjs/cli`/babel build-and-test toolchain) with no production runtime exposure — none ship in the CLI, MCP server, or Core API's built artifacts. Deliberately left open: a real fix is either a new, targeted nested `overrides` entry (following this repo's own established pattern of scoped overrides like `overrides.eslint.ajv`, but unverified here against every workspace's actual lint/test run) or a major version bump of core tooling across roughly 10 workspaces — both are the kind of hard-to-reverse, broad-blast-radius change to a monorepo-wide shared lockfile that should not be pushed unilaterally at the tail of a long session without dedicated regression testing across every consumer.
