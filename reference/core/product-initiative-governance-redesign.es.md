...
Coherentes con el SPINE (mismas firmas de entidad) y con los tipos canónicos existentes `Verdict` (`packages/core-domain/src/domain/verdict/verdict.ts:14`) y `PhaseId` (`packages/core-domain/src/domain/sdlc/phase-id.ts:14`). Se exponen como DTOs de request/response en Core-API; las entidades de dominio (`Producto`, `Iniciativa`, etc.) viven en `@beyondnet/evolith-core-domain` y son la fuente de verdad.
