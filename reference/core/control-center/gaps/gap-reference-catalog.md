...
- **Provenance:** Registered 2026-07-31 from CI run `30631939687` and reproduced locally the same day. Id assigned by union of `origin/main..origin/develop` and the open pull requests (`#325` and the dependabot set), whose maximum was `GT-645`. Split from [`GT-647`](./gap-reference-catalog.md#gt-647) deliberately: the WARNs in that log are a *symptom* of this per-request load but are independent corpus defects, and folding them together is the one-gap-one-claim failure [`GT-639`](./gap-reference-catalog.md#gt-639) exists to prevent.
- **Acceptance criteria:**
...
**Title:** Three corpus documents are reported broken on every load, the phase-gate stage has evaluated nothing since 2026-07-04, and every single-argument log line is printed twice

- **Purpose:** Make the evaluation corpus load cleanly and completely — every document checked against the contract it actually declares, every authored gate reachable, and one log line per log call.
- **Evidence:** Three faults surfaced together in the `core-api.log` of CI run `30631939687`; each is independently real, and the second is the serious one. **(1) THREE `*.rules.json` FILES FAIL THE STANDARD RULESET SCHEMA AND ARE SKIPPED ON EVERY LOAD**, with `must have required property 'rules' / 'principles'`: `architecture/topology-recommendation.rules.json`, `infrastructure/helm-enforcement.rules.json`, `infrastructure/opa-sidecar-bundle.rules.json`. Nothing is wrong with any of them — they are *different document kinds* that share the `*.rules.json` suffix. The first is the ADR-0104 advisory recommendation catalogue, which has its own `topology-recommendation.schema.json` and its own consumer (`TopologyRecommendationService`, `topology recommend`); the other two are single-rule declarations in the `rule-definition.schema.json` shape, enforced by guard `29-validate-opa-sidecar-bundles.mjs` and their paired Rego policies rather than by the rule engine. The loader had no vocabulary for "not a corpus ruleset", so it checked each against a schema it was never meant to satisfy and reported the result as breakage. **Worse, and unnoticed until this row: all three also failed the schema they themselves DECLARE**, because nothing ever validated them against it — `severity: "error"` is absent from that schema's enum, the field is `title` and the files wrote `name`, the `validation` block the files have carried from the start was never modelled at all, and the `$schema` paths were left stale by the `reference/` reorganisation (`../../schema/…` resolves to a directory that does not exist). Their `validation.target` globs point at `reference/infrastructure/…`, a tree removed by `f0d01911`. **(2) `reference/governance/sdlc/phases/` DOES NOT EXIST, AND HAS NOT SINCE 2026-07-04.** The five `phase-f*.json` files were deleted by `f0d01911` — *"chore(taxonomy): remove leftover old directories under reference/"* — which removed the whole `reference/governance/` tree; `GT-461` (`061eacbd`) later re-created `gates/` **at that same path** and did not re-create `phases/`. This is not a cosmetic warning: `loadGatesForPhase` resolves gates *through* the phase index, so `loadAllPhases()` returning `[]` makes **all five authored gate files unreachable**. For roughly four weeks the phase-gate stage of `SatelliteEvaluationPipeline` produced zero gate results and reported no error — a governed evaluation silently skipped the half of the pipeline the SDLC gates live in, and the top-level verdict was computed without them. Confirmed against a running Core: with the data restored, `POST /api/v1/evaluate` returns `gate-f1` alongside `general-rulesets` where `origin/develop` returns only the latter. **(3) `NestLoggerProvider` FORWARDS ITS OPTIONAL `context` UNCONDITIONALLY.** Nest's `Logger` reads `...optionalParams` positionally, so `this.logger.warn(message, undefined)` is not the same call as `this.logger.warn(message)` — the explicit `undefined` becomes a second thing to print. Every single-argument `warn`/`debug`/`info`/`error` from the domain therefore emitted a paired `WARN undefined` line: **44 of them across 11 requests** in the baseline reproduction, doubling the volume of everything the Core writes and putting a meaningless line next to every real one.
- **Component:** `Core Domain` · **Criticality:** P1 · **Complexity:** S
- **Provenance:** Registered 2026-07-31 from the same `core-api.log` that produced [`GT-646`](./gap-reference-catalog.md#gt-646), and kept separate from it on purpose: those WARNs are how the per-request load was *noticed*, but they are corpus and logging defects that would survive any amount of caching. Fault (2) was traced by `git log --diff-filter=AD` rather than assumed — the files existed, and the commit that removed them says it was clearing leftovers.
- **Acceptance criteria:**
