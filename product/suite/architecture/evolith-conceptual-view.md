...
| Agent Runtime API (`src/apps/agent-runtime-api`) | Governed agent orchestration over `src/packages/agent-runtime` | NestJS, `@Sse`, OTLP/HTTP exporter | `POST /v1/agent` (`handle`, `converse`), `POST /v1/agent/stream` (SSE) · API-key/JWT guard. `/hermes` is only a deprecated alias of `/converse` | [`src/apps/agent-runtime-api/package.json`](../../../src/apps/agent-runtime-api/package.json) |
...
```bash
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu --hide-scrollbars --window-size=1600,1442 --screenshot=/tmp/evolith-conceptual-view.png "file://$PWD/product/suite/architecture/assets/evolith-conceptual-view.svg"
```
