...
| [`GT-380`](./gap-reference-catalog.md#gt-380) | R4 — OPA `input.context` aligned to `EvaluationContext` (re-anchor `dod.rego` off `input.story.*`; remove story artifacts from `phase-gates` mandatoryEvidence; multi-tenancy MTN-09..11 + ABAC scoping; Native+OPA parity 0 drift) | `OPA` | Cross | P1 | M | `PENDING` |
