...
- **Principal:** `XL` · **Interest:** `LOW` · **Basis:** `estimate`
...
- **Acceptance criteria:**
  - [x] Portfolio views and per-tenant packages available in the Tracker.
  - [x] Introduces no inverse Core→Tracker dependency.
- **Criterion 1, first half SHIPPED 2026-08-02 — per-tenant governance packages** (`evolith_tracker#115`). The row's problem statement is that «governance cannot be packaged per customer»: a tenant configured it piece by piece, one `GatePolicy` per phase and one `ArtifactFieldSchema` per artifact type, with no way to take that configuration whole, version it, review it as a unit or move it to another tenant. `GET /governance-packages/export` now produces a named, versioned, portable document and `POST /governance-packages/apply` installs one. **Four decisions worth arguing with.** (1) A package is a DOCUMENT, not a persisted aggregate — storing it would add a third copy of data that already lives in two places, and this repository has spent the week repairing exactly that failure; no table also means no migration and no drift. (2) It carries **no tenant id anywhere**, at any nesting level, and the destination is always the SESSION tenant: a package that dragged its source tenant into a target is precisely the isolation leak the product forbids. Enforced by reflection over the types rather than over a call, so adding the field breaks the test even if nobody writes a request that uses it — verified by adding it and watching it go red. (3) Export is deterministically ordered, because two exports of the same state must produce the same document or no one can diff two packages or review a governance change in a pull request. (4) Apply refuses an invalid package ENTIRELY before touching anything, but does not stop at the first failure once it starts — a partially applied package is a fact worth seeing whole, and stopping early would leave a state the report does not describe. **The product ships the MECHANISM and no content:** per `T-056` a canned «ISO 27001 package» would be engine code holding an opinion that belongs to the tenant, and a test asserts the repository contains no applicable package. 13 tests.
- **Criterion 1, second half was ALREADY MET and is credited rather than rebuilt.** `TowerMd3` is the executive view — «Control Tower», subtitled *live SDLC governance across the portfolio*, with a **Portfolio flow** section plus active-SDLC, gates-passed, blocked-gates and approved-initiative counters over the session tenant's initiatives and executions. Rebuilding it would have been the eighth instance this week of implementing something that already existed.
- **Criterion 2 holds by construction:** every file added is in `evolith_tracker`, and nothing in the Core references the package types. The row's own risk note — «effort dispersion; it belongs to the Tracker, not Core» — is respected: what landed is the packaging mechanism, not the marketplace adapters of the title, which the acceptance criteria never asked for.
- **Dependencies:** none.
- **Status:** `DONE` — 2026-08-02. **Closed on its CRITERIA, not on its title.** Both are met: the portfolio views already existed (`TowerMd3`) and the per-tenant packages landed in `evolith_tracker#115`; nothing in the Core references the package types. **The marketplace adapters in the title were NOT delivered** and were never in the criteria — registered as `GT-651` so the closure does not bury them.
...
- **Principal:** `XL` · **Interest:** `HIGH` · **Basis:** `estimate`
...
- **Principal:** `XL` · **Interest:** `SEVERE` · **Basis:** `estimate`
...
- **Acceptance criteria:**
  ...
  - [x] One chaos drill kills a dependency mid-run and the recorded behaviour matches what ADR-0011 declares.
  - [ ] RTO and RPO are MEASURED on a real DR restore and written into ADR-0013, replacing the current unquantified claim.

- **Criterion 3 CLOSED 2026-08-02, by making the declaration checkable instead of loosening the check.** The blocker was named honestly a moment earlier: «the recorded behaviour matches what ADR-0011 declares» is unverifiable against an ADR that declares no numbers. So ADR-0011 gained a «Calibration in force» section. **Three of its four values were not invented — they are what the code has been running on** (`failureThreshold` 5, `resetTimeoutMs` 30 000, `timeoutMs` 10 000, from `circuit-breaker.ts`), and recording them promotes a fact that lived only in a default object into the governance record. The two recovery budgets are derived from measurement: **two independent CI runs of three drills each, six recoveries, 6/6 successful**, giving mean MTTRs of 9 219 ms and 10 971 ms and first-verdict-under-load of 71 793 ms and 72 361 ms. Budgets set at ~2× the worse observation — 25 s and 150 s — with the headroom argued rather than assumed: on shared runners a budget pegged to the observed value fails on noise and teaches everyone to re-run the job, which is how a red check becomes a ritual. The `chaos-drill` job now asserts both, and asserts that it measured anything at all. The error rate during the outage (~33 %, 247/745 and 235/733) is deliberately NOT bounded — it is the expected observation of a drill that kills a dependency on purpose, and bounding it would fail the drill for succeeding. Assertion logic exercised against the real run data and against three failure shapes (MTTR degraded to 40 s, no verdict ever returned, no MTTR log at all); it goes red on all three.
- **Criterion 4 remains the only one open, and is not blocked on this repository.** RTO and RPO need a real DR restore on a running deployment, which is [`GT-435`](./gap-reference-catalog.md#gt-435) / [`GT-448`](./gap-reference-catalog.md#gt-448) territory: nothing is promoted to a server customers reach. The drill's own summary already refuses the confusion — it publishes MTTR and says in the step output that a container-restart drill on one CI host is never RTO/RPO.
- **Progress 2026-08-02 — criterion 3's drill existed and COULD NOT FAIL.** The `chaos-drill` job kills redis under load and measures MTTR to the first governed verdict, and both of its scripts already end in an assertion: `kill-under-load.sh` fails when the system never returns a verdict, and `recovery-time.sh` exits 1 under the comment «NO run recovered a verdict — this is itself a finding». **Both were disarmed by a pipe.** The workflow declared no `shell:`, so every `run:` step got Actions' implicit `bash -e {0}`, which does NOT set `pipefail`; the two steps that pipe a drill into `tee` therefore took `tee`'s exit status. A total outage that never came back would have published its own evidence to the step summary under a green tick. Fixed by declaring `defaults.run.shell: bash`, which is what gets `-eo pipefail`. Added alongside it: a step that fails when the drill issued **zero requests**, because a system nobody called looks healthy and «it recovered» means nothing without a denominator.
- **Criterion 3 stays open, and the reason is in ADR-0011 rather than in the harness.** «The recorded behaviour matches what ADR-0011 declares» cannot be checked numerically: the ADR declares no thresholds, and says so itself under Negative — «requires sophisticated parameter calibration (how many errors before break, timeout limit, restore cooldown)». What it DOES declare in §1 — that breaker state lives in a shared Redis cluster — is knowingly deviated: `circuit-breaker.ts` is process-local, for reasons `GT-560` established. That deviation was written in a source comment and **not** in the ADR, so the governance record said one thing while the code did another. It is now recorded in ADR-0011 under «Deviations in force», with its cost stated: with N replicas a failing dependency absorbs up to N × `failureThreshold` calls instead of `failureThreshold`. Closing this criterion needs the calibration the ADR never fixed; the drill's published MTTR is the input for it.
...
- **Residual (criterion 2 — owner's infra). RE-DIAGNOSED 2026-08-01; the previous note was stale and would have sent a reader to change two secrets that are already correct.** It said `curl` failed with `Could not resolve host` because the deploy-hook secrets held a UUID or a path instead of a full URL. That is no longer what happens. Measured on run `30713787775` (push to `main`), the failure is `curl: (28) Failed to connect to 72.60.63.240 port 8000 after 133138 ms: Couldn't connect to server` — the secret **is** a full URL, it resolves to an address, and the connection times out. **Verified from a second vantage point so the cause is not attributed to the runner:** from a developer machine the same host answers on neither `8000`, `443` nor `80`, and does not answer ICMP at all (3 packets, 100% loss). So this is not a GitHub-Actions egress or firewall-allowlist problem — the VPS is unreachable outright. The blocker is therefore one of: the host is down, its address changed, or its firewall drops everything; none of the three is decidable from this repository and none is fixed by editing a secret. **Criterion 1 is green and stays green:** run `30713787775` built and pushed all three images — `core-api`, `mcp-server` and `agent-runtime` — to GHCR. What is missing is only the last hop. Stays `IN-PROGRESS` until a reachable host makes a green deploy possible. *(Separate, non-GT-324: the legacy CLI→Docker Hub `docker` job fails because sdk/cli has no per-package lockfile for its standalone Dockerfile.)*
- **Deploy job HARDENED 2026-08-02, while criterion 2 stays owner-blocked.** Each service used to check its own hook and, when unset, warn and `exit 0`. That is right for «not configured yet» and wrong for «half configured»: set the token, typo one hook, and CD reports success while that service never deploys — and the only symptom is an old image serving traffic nobody is watching. Nothing configured is now a skip; **PARTIALLY configured is a failure** that names how many of the four values are present. The weakness was found while writing the Tracker's equivalent job (`evolith_tracker#114`); leaving this one weaker after documenting why would have been the same staleness this board tracks, so both repositories now fail the same way. Also hardened: `curl --fail` is explicit, because without it curl exits 0 on a 4xx and a REJECTED hook reads as a successful deploy; and all three hooks are attempted before failing, so one bad hook does not hide the state of the other two. Classification exercised in all four states — nothing, complete, and two shapes of partial. **And the job now says what it did not prove:** Coolify is asynchronous, so a green step means the request was accepted, never that the new image is serving. GT-448 asks for a recorded run against the live service, and this job cannot provide that evidence — written into its step summary rather than left for a tick to imply.
- **Done when:**
...
- **Provenance:** Product maturity audit of 2026-07-26 (multi-agent with adversarial verification). Full detail, evidence and systemic context in [product-maturity-audit-2026-07-26.md](../maturity-reports/product-maturity-audit-2026-07-26.md).
- **Acceptance criteria:**
  - [x] A real `tools/call` over stdio against the published tarball returns a verdict, not FORBIDDEN.
  - [x] The documented escape flags either work or are removed — no flag that silently does nothing.
  - [x] The CI smoke asserts a verdict, not the mere existence of a field.
- **Closure evidence:** Verified against an `npm pack`ed tarball installed OUTSIDE the workspace, so no symlink could stand in for the shipped files — the GT-625 illusion. With the default configuration the packed server returns a real verdict over stdio (`gateId` `business-sign-off`, `verdict` `failed`); the original "47 of 47 FORBIDDEN" did not reproduce. Under `NODE_ENV=production` it WAS forbidden, and the cause is packaging rather than policy: `policy.wasm` is a gitignored build artifact written outside the mcp-server package, so no `files` setting can reach it and a published consumer inherited a fail-closed rule it had no way to satisfy; `EVOLITH_ABAC_POLICY_PATH` makes that posture satisfiable today. **Shipping the compiled bundle inside the package remains a release-pipeline decision: documented, and deliberately NOT done here.** Two defects were repaired on the way — `PhaseGateValidatorService.findCorePath` still resolved `<repo>/src` after the `src/` refactor (a GT-632 sibling), and `corePath` was read by `evolith-gate-evaluate` while being undeclared in its `inputSchema`. The old smoke asserted `envelope.success !== undefined`, true of every response, and was observed printing `tools/call OK` while receiving `RULESET_NOT_FOUND`; the replacement (`src/sdk/cli/examples/gate-verdict.assert.js`, unit-tested in `gate-verdict.assert.test.mjs`) asserts a real gate verdict and was proven falsifiable three ways, including a live run against the pre-fix dist. **Caveat on criterion 1:** it is ticked against a locally packed tarball, not against the registry — 1.1.0 predates every fix here, and re-publishing is `GT-570`.
...
- **Principal:** `M` · **Interest:** `HIGH` · **Basis:** `estimate`
- **Provenance:** Product maturity audit of 2026-07-26 (multi-agent with adversarial verification). Full detail, evidence and systemic context in [product-maturity-audit-2026-07-26.md](../maturity-reports/product-maturity-audit-2026-07-26.md).
- **Closure (2026-08-01, `a52cc3a7`):** The evidence-command corpus now reports **0 dead references** locally and a clean-checkout ratchet basis of **17** generated/gitignored referents (`dist/`, `node_modules/`, `.harness/bin/`). The three remaining search-with-no-match outcomes were converted into assertable positive commands, including a real residual fix for Grafana's compose password placeholder. CI now runs the board evidence guard as governed execution: `--execute --strict --max-dead 17`. Verified locally with 109 unique candidate commands, 109 exit 0, 0 non-zero and 0 inconclusive outcomes.
- **Acceptance criteria:**
  ...
  - [x] 100% of the board's `validationCommands` are executable and green in CI.
...
**Title:** Two unreconciled artifact vocabularies make an artifact catalog unpublishable
...
- **Principal:** `M` · **Interest:** `HIGH` · **Basis:** `estimate`
- **Acceptance criteria:**
  - [x] An ADR states which corpus is canonical for required artifacts, and resolves the `Coverage Report` phase disagreement explicitly rather than by picking one silently.
  - [x] One catalog remains. The losing corpus is deleted or derived from the winner — two files that agree today drift tomorrow, which is the defect this row records.
  - [x] The catalog is reachable over HTTP and returns, per phase, every required artifact with its canonical `$id` where one exists and its `producedBy` where the artifact is tool output.
  - [x] A guard fails when the published catalog and the corpus disagree, verified by making them disagree.
  - [x] `evolith_tracker` can replace `StandInPhaseArtifactProfileSource` with a `core-sync` source, and its `GAP-004` records the Core half as unblocked.
- **Correction, 2026-08-02, same day this row was filed.** The row said the HTTP surface "reads a third file with no artifacts at all". **That is wrong.** `src/rulesets/sdlc/phase-gates.rules.json` carries the same 24 artifacts, under the key `mandatoryEvidence` rather than `requiredArtifacts` — it is a second COPY of the gate corpus, not an unrelated file. The corrected finding is worse than the original: two hand-maintained copies of the same catalog, one feeding the evaluator (`phase-gate-validator.service.ts`, `gate-registry.service.ts`) and one served to satellites, **already drifted in seven artifacts**. `#378` wired schemas for `Discovery Canvas`, `Ballpark Estimation`, `Bounded Context Map` and `Definition of Done Checklist`, and declared `CI Pipeline`, `Coverage Report` and `Pyramid Distribution` as tool output — every one of those seven edits reached the evaluator's copy and none reached the copy anyone can query. The Core evaluated against one answer and published another, with nothing red anywhere.
- **Also corrected: the schema paths are NOT inconsistent in a way that matters.** `../schema/prd.schema.json` resolves correctly from `src/rulesets/sdlc/`; only the other copy's paths were broken before `#378`, because `../schema/` from `reference/governance/sdlc/gates/` pointed at a directory that does not exist. Each file spells the reference relative to itself and both resolve. Worth stating because the obvious "fix" — rewriting one set of paths to match the other — would be a change with no cause.
- **Partial closure 2026-08-02 — the drift is closed and guarded, the reconciliation is not.** The seven missing fields were synced into the served copy, preserving its own relative-path convention. `57-validate-gate-corpora-parity` now fails when the two copies disagree on which schema an artifact has or on whether it is tool output, comparing the RESOLVED BASENAME rather than the path string so that two valid spellings are not reported as a difference. Eight self-tests, six negative; verified failing by reintroducing the drift, and registered with `43-validate-guard-negative-fixtures` (46 guards, all observed refusing an empty fixture). **What remains is the whole of the original row:** there are still two hand-maintained copies plus a third, genuinely different vocabulary in `UNIVERSAL_PHASE_ARTIFACTS` (18 slugs over three phases), and deciding which is canonical — including where `Coverage Report` belongs — still needs an ADR.
- **ADR drafted 2026-08-02 — [`ADR-0125`](../../architecture/adrs/core/0125-single-artifact-registry.md), status `Proposed`.** It resolves the `Coverage Report` question by rejecting its premise: the artifact is required at `construction` AND expected at `quality`, both true, and neither file could say so — so the registry records `phases` as a list. Slug is the identity because it is already the machine vocabulary of the topology manifests, the completeness service and the Tracker; the human name becomes a display `label`. `phase-gates.rules.json` becomes generated and `57-validate-gate-corpora-parity` is deleted with it, since a guard kept past its cause becomes noise. Acceptance criterion 1 is met by the draft; the rest await the Board.
- **ADR-0125 ACCEPTED 2026-08-02.** The decision is in force: one artifact registry keyed by slug, with `label` demoted to display, `phases` as a LIST so `coverage-report` can be required at construction and expected at quality without either file having to lie, and `binding` versus `advisory` preserving the distinction between gate evidence and completeness rather than flattening it. **It is accepted as a decision, not as delivered capability**, and the ADR declares `implementation-status: none` so it appears in the same census as `GT-607` — Accepted ADRs with no implementing code — instead of quietly reading as done. What remains for this row is the migration itself, and the ADR names the part that is easiest to skip: **the losing corpus must be deleted or derived**, because two files that agree today drift tomorrow — the defect this row was opened for, which `57-validate-gate-corpora-parity` currently manages rather than ends.
- **Migration STARTED 2026-08-02 — the registry exists and its first consumer derives from it.** `src/rulesets/sdlc/artifact-registry.json` declares **33 artifacts**: the 24 the gates require, classified `binding`, plus the 9 that only the completeness constant named, classified `advisory` **because no gate requires them** — that is evidence, not a default, and promoting any would change what blocks a release. Exactly one artifact carries two phases, `coverage-report` at `construction` and `quality`, which is the disagreement ADR-0125 was written around. **A decision the ADR left open is fixed here:** the phase vocabulary is the Core's own (`discovery`, `design`, `construction`, `quality`, `deployment`, plus `intake` declared and currently empty), because `DownstreamPhase` is live code keyed on three of them; a satellite whose phases are named differently maps them as a written decision, as `evolith_tracker` already does for artifact kinds. `UNIVERSAL_PHASE_ARTIFACTS` is now GENERATED from the registry, registered as the first link of the derived-artifact chain, which replays to a fixed point across 7 links.
- **The generated constant changed behaviour, and the direction matters.** SIX artifacts entered and none left: `coverage-report` and `documentation-delta` at construction, `acceptance-validation`, `integration-evidence` and `pyramid-distribution` at quality, `on-call-handoff` at deployment. **Every one is required by a gate and was missing from the hand-written list**, so the advisory completeness figure had been flattering — a phase could report complete while missing something its own gate demands. One evaluation in the test corpus drops from 88% to 64% for exactly that reason, and the three newly-missing artifacts are named in its expectation rather than hidden behind a number. 1704/1704 core-domain tests pass.
- **A FOURTH corpus was found that the ADR did not know about.** `src/rulesets/schema/phase-artifact-registry.json` catalogues artifact KINDS including the topology-derived ones, and a CoC-completeness test asserts every universal kind appears in it. Five of the six new entries had to be added there too. The ADR's context named three corpora; there are four, and the migration's definition of done has to account for it.
- **Still open, and it is the larger half:** the gate files still carry `schemaRef`/`producedBy` instead of referencing the registry by id, `phase-gates.rules.json` is still hand-maintained rather than generated, and the registry is not yet published over HTTP. `59-validate-artifact-registry` makes the intermediate state safe by failing when the registry and the gate corpus disagree — in both directions, verified by making them disagree — and it dies together with `57` when the gates derive from the registry.
- **A derived artifact was found that cannot be regenerated anywhere but CI, and it cost two failed runs to see.** `reference/core/interfaces/how-to-{construction,qa}.md` embed BOTH deterministic content — the artifact lists, which genuinely changed here, construction going 5 → 7 — and captured findings whose rule ids differ by environment: the runner produces `DRIFT-01` where this machine produces `INH-03`. Regenerating locally therefore fixes the first half and breaks the second, forever. **The mistake was mine and it is worth naming:** regenerating twice on one machine and getting a byte-identical file proves stability WITHIN an environment, and it was read as determinism ACROSS environments. The fix was to take the committed files and edit only the artifact lists. The defect belongs to the artifact, not to this change — a derived document that mixes deterministic content with environment-dependent capture output can only be regenerated in one place, and nothing declares that.
- **The migration hit a decision that is not an engineer's to make, and it stopped there.** Generating `phase-gates.rules.json` from the gate corpus revealed that the two copies disagree about far more than the seven schema fields already recorded: they carry **different governance facts**. Measured 2026-08-03 — `f2` accountableRole `Architect` vs `Software Architect`; `f3` accountableRole `Engineering Lead` vs `Tech Lead`; `f3` waiverAuthority `Architecture Board` vs the same *plus Executive Risk Acceptance for CVEs*; **`f4` waiverAuthority `Release Manager` vs `Architecture Board`**; `f5` accountableRole `Release Manager` vs `DevOps Lead`; **`f5` waiverAuthority `Executive Sponsor` vs `Technology Director`**. Each gate's `description` also differs in kind: the corpus describes the PHASE, the served copy describes the gate's EXIT CONDITION, and both are legitimate content that no merge can invent. **Who may waive a quality gate is a governance answer, and generating from either source would publish one as though it had been decided.** The generator is written, tested against the current file and deliberately NOT wired; it waits for the decision rather than making it.
- **What did land, and it is safely partial.** Every field moved to the place that owns it: `templateRef` to the registry, because where an artifact's authoring template lives is a property of the artifact and not of each gate that requires it — which is exactly how the seven-field drift became possible; `status` and `waiverRequiredFields` to the gate corpus, because they are about the gate. `57` and `59` both stay green, so the three surfaces still agree on everything they used to agree on.
- **DECIDED 2026-08-03 by product direction: the evaluator's copy always rules.** The generator is wired and `phase-gates.rules.json` is now DERIVED, so the two copies cannot disagree by construction. **Six governance answers changed on the published surface, and that is the point rather than a side effect** — the surface now says what the engine enforces: `f2` accountable `Software Architect` → `Architect`; `f3` accountable `Tech Lead` → `Engineering Lead`; `f3` waiver loses the CVE clause; **`f4` waiver `Architecture Board` → `Release Manager`**; `f5` accountable `DevOps Lead` → `Release Manager`; **`f5` waiver `Technology Director` → `Executive Sponsor`**. **`exitCriteria` was preserved, not dropped.** The served copy's per-gate text described what a gate demands to PASS while the corpus `description` describes the phase — a different question, not a conflicting value — so it moved INTO the gate corpus, which puts it under the same authority the rule names instead of making it an exception. Integrity audited rather than assumed: 5 gates, 24 artifacts, identical artifact-per-phase membership, 14 schemas and 3 tool-output declarations before and after.
- **`57-validate-gate-corpora-parity` is DELETED with this change**, tests and all. Its subject was the disagreement between two hand-maintained copies, and one of them is now generated — a guard kept past its cause becomes noise, and noise trains people to skip red. `59-validate-artifact-registry` survives because its subject is different (registry against gates), and the condition for its own deletion is written into it: when the gates reference the registry by id and stop carrying `schemaRef`, there is one declaration and nothing left to compare.
- **What remains of the migration:** the gates still carry `schemaRef`, so registry and gates are still two declarations of the same fact; and the registry is not yet published over HTTP. Chain: 8 links, at a fixed point. 1704/1704 core-domain tests.
- **MIGRATION COMPLETE 2026-08-03.** The gates now reference the registry BY SLUG — `artifactId` — and no longer carry `schemaRef` or `producedBy`. `GateRegistryService` resolves those from the registry at load time, so the seven-field drift of `#378` is no longer detectable-but-possible: it is **unrepresentable**, because there is one place to write the fact. The generator and guard `59` were switched from label lookup to slug lookup, and the generated corpus came out byte-identical, which is the proof that the two keys named the same things. **The registry is published:** `GET /api/v1/phases/artifacts` and `GET /api/v1/phases/{phase}/artifacts`, carrying each artifact's schema `$id` — never a repository path — or its `producedBy`. Exercised against the real corpus rather than only compiled: 33 artifacts, `prd` resolving to `https://evolith.dev/schema/prd.schema.json`, `coverage-report` returning both of its phases, and an unknown phase yielding **0 artifacts** rather than the whole registry, because falling back to everything would read as «this phase requires all of it».
- **`evolith_tracker` GAP-004's Core half is unblocked.** The satellite ships `StandInPhaseArtifactProfileSource`, a hand-built mirror stamped `core-standin`, precisely because there was nothing to consume. There is now, and `IPhaseArtifactProfileSource` already reserves the `core-sync` provenance for it.
- **What the migration cost, stated once.** One consumer's cached-read count moved 2 → 3 (the registry is read once and cached alongside the gates); six governance answers changed on the published surface; six artifacts entered the advisory completeness set and lowered two published figures; and a fourth corpus nobody had named had to be completed. Every one of those was a pre-existing disagreement that the single declaration forced into the open.
- **Status:** `DONE` — 2026-08-03.

#### GT-651

**Title:** Marketplace-style adapters — the half of GT-532 its closure must not bury
...
- **Acceptance criteria:**
  ...
  - [x] `Secret Detection (gitleaks)` is a required context on `main` and `develop`. **Deliberately last, and sequenced:** promoting it before a green run on both branches would have made every open pull request unmergeable with nothing to point at — the exact deadlock the `paths:` filter comment at the top of `sdk-cli-ci.yml` records from PR #218. Flipped by the owner on 2026-08-03 after the job reported green on `develop` (run 30867424760) and on `main` (run 30868306980). **Read back from the API rather than assumed:** both branches now carry 8 required contexts, up from 7, with `enforce_admins: true` — so the promotion is backed by configuration, not by prose.
...
- **Acceptance criteria:**
  ...
  - [x] A CLI command reports a confusion matrix, κ and a CI95 interval inside the ADR-0073 envelope.
  - [ ] A per-ruleset precision figure is published for the deterministic rules already shipping.
- **Criterion 2 CLOSED 2026-08-02 — the instrument, built before its data on purpose.** `evolith calibrate report --labels <path>` reads adjudicated decisions (JSON or JSONL) and reports, per ruleset, a confusion matrix, the false-block rate, precision, recall and Cohen's κ inside the ADR-0073 envelope. The arithmetic lives in `core-domain/domain/calibration` as pure functions and is tested against WORKED EXAMPLES with published answers — the textbook 2×2 giving κ=0.4, and the Wilson interval for 0/10 giving an upper bound of 0.2775 — rather than against itself, which is the unfalsifiable measurement this row exists to replace. **Four decisions worth arguing with.** (1) BLOCK is the positive class, so a false positive IS a false block, and the rate divides by the gate's blocks rather than by every decision — otherwise a gate that blocks nothing earns a flattering figure by doing nothing. (2) Nothing returns a bare point estimate: every rate travels with its Wilson interval and its denominator, because 0/4 and 0/400 are the same number and completely different claims. Wilson over the normal approximation precisely because the normal collapses to [0,0] at zero successes, claiming certainty from no evidence. (3) κ is `null`, never 1, when the sample cannot support it, and a gate that blocks nothing scores **exactly 0** despite 95 % raw agreement. (4) A rate is flagged not-quotable below 30 adjudicated blocks — the interval is still published, what is withheld is the headline. **It emits no verdict.** A calibration report is a measurement, not a gate; wiring a high false-block rate to `BLOCKED` would make every honest measurement look like a regression. Verified end-to-end against a hand-written corpus and on all four exit paths (0 report, 1 unreadable corpus, 3 malformed row, 3 unknown action). 24 tests.
- **Criteria 1 and 3 stay open and are NOT blocked on code.** They need labels: criterion 1 a hand-labelled set with the human-to-human agreement ceiling, criterion 3 a published per-ruleset precision. The parser refuses a row whose `humanBlocked` is missing rather than defaulting it to `false`, because an unadjudicated row silently entered as a true negative drifts every figure in the flattering direction. The row therefore stays DEFERRED on its original reason — no organic label corpus until something runs in production (`GT-435`/`GT-448`) — with the difference that the figure is now derivable the day labels appear instead of retrofitted afterwards.
...
- **Acceptance criteria:**
  ...
  - [x] A governance rule fails when receipts do not verify, with a negative test. **Verified 2026-08-01:** `audit-transparency.rule.ts` ships four sub-rules with **nine** negative tests — no ledger at all, an empty ledger (rather than passing vacuously), a decision edited after signing, a receipt signed by an unanchored key, an entry deleted from the middle of the chain, the development signer even when every receipt verifies, a verifier that fell back to keys carried inside the ledger, and the Issuer and the Transparency Service sharing one key. It also reports the offending ledger LINE, not just the file. The row carried this criterion as open while it had been satisfied for some time, which is the kind of staleness that misdirects the following week's priorities.
  - **Criterion 1, measured again 2026-08-02 — the wire was complete and the SWITCH was missing.** The Tracker side shipped (`evolith_tracker#111`): a decorator over `IAuditEntryRepository` — chosen over touching each caller, so the four paths that already record decisions sign without edits and so does the fifth somebody adds tomorrow — plus Merkle, COSE_Sign1, an Ed25519 signer and the JSONL ledger, all under cross-language interop in CI. And still **nothing was signed anywhere**, for a reason that was not in the code: `Transparency` appeared in no `values.yaml`, no configmap and no deployment, so an operator holding real keys could only invent a `--set` over a non-existent key, which Helm accepts in silence and which enables nothing. Now declared, OFF, with both Ed25519 seeds read from an existing Secret, the ledger on a PVC whose absence FAILS the render, and an operator procedure in the chart README. Five tests read the CHART rather than the code — what was missing was the operating surface, and a test over the classes would have stayed green the whole time this was open; the off-by-default one was verified by flipping it and watching it go red.
  - **Still NOT met, and deliberately left unticked.** No environment enables it: that needs two distinct Ed25519 seeds in a secret store plus a persistent volume, and key custody is an owner decision, not a code change. The honest state is that this went from *impossible without touching code* to *a deployment decision* — which is progress worth recording and not a closure. The signing point remains the Tracker per the 2026-08-01 decision; the Core holds the verify half and the governance rule.
  - **Where criterion 1 actually stands, measured 2026-08-01.** The infrastructure is complete and tested — Merkle tree, CBOR/COSE, an Ed25519 statement signer AND verifier, `verifyReceiptChain`, a JSONL ledger, and a `transparency-e2e.spec`. What is missing is the WIRE: `AuditService` takes the recorder as an OPTIONAL constructor parameter (`transparency?`), and `new AuditService(...)` appears **nowhere outside test files**. Nothing in production constructs it, so nothing signs. The consequence is that criterion 3 already punishes a ledger that fails to verify while no ledger is ever emitted — the rule has nothing to punish. **Decision 2026-08-01: the signing point is the Tracker**, not the Core (which is a stateless evaluation engine per `ADR-0101` and cannot hold a ledger) and not the CLI (which owns the verify half). The Tracker is where gate decisions and the audit ledger already live, and where [`GT-605`](./gap-reference-catalog.md#gt-605) just landed the evidence graph those statements would anchor to.
...
- **Acceptance criteria:**
  ...
  - [x] No rule marked `blocking` can return `skipped`; that combination fails the run.
  - [x] The `rulesChecked`/`rulesTotal` ratio is published per ruleset.
- **Closure evidence:** `RuleEvaluationEngine.toValidationIssues` now emits a `blocking` ValidationIssue whenever a rule declared `blocking` returns `skipped`, so the combination fails the run (`blocking-skipped-invariant.spec.ts`). It is deliberately NOT a thrown error: `RulesetValidatorService.validate` catches engine exceptions and downgrades them to `logger.warn`, so a throw would have been silently swallowed by the only product caller. The rule's own outcome stays `skipped` on purpose — promoting it to `failed` would move it into `rulesChecked` and re-inflate the very coverage GT-569 made honest. Enforcing it exposed **181 rules declared `blocking` that never execute**: 96 were auto-generated ADR placeholders whose own `validationQuery` ends "Concrete checks to be wired into the harness" (the generator asserted `blocking: true` on a check it never wired, and now emits `blocking: false`), and of the remaining 85, **12 were closed for real** — 8 config-shaped (ED-R04, ED-R05, ED-R06, DAM-R05, MTN-05, GIT-08, SEC-RL-01, SEC-RL-02) and 4 module-boundary (HXA-01, HXA-02, HXA-04, HXA-05). The pinned count is now **73**, recomputed against the real corpus rather than assumed; the pin was perturbed to 74 and observed failing. **Two findings worth keeping, both the same pattern as the gap itself.** The four HXA rules were dead because `DiskRulesetRepository.normalizeRuleset` never copied the `enforce` field: the schema has accepted it since GT-516, ADR-0002 authors six blocks and the PolicyCompiler compiles them, but normalization dropped it, so `rule.enforce` was always `undefined` and the entire enforcer subsystem was unreachable for lack of data, not for lack of wiring. And `rule-corpus-triage.spec.ts`'s own loader dropped `enforce` too, under a docstring claiming it loads "exactly as `DiskRulesetRepository` does" — the measuring instrument was measuring a different corpus than the one production evaluates. **Two deliberate non-adoptions.** `dependency-cruiser` was rejected: the Core is a stateless engine reading satellites through an overlay filesystem a subprocess cannot see, and the tool's config matches `node_modules/`, so an uninstalled satellite would yield zero violations and PASS four blocking rules — a false pass is worse than the skip it would replace. OCB-02 was not closed: no artifact anywhere carries an `availability` marker, so the rule quantifies over the empty set and can only pass; that is recorded as three executable tests instead of prose.
...
- **Principal:** `M` · **Interest:** `MED` · **Basis:** `estimate`
- **Provenance:** Derived from the 2026-07-26 product maturity audit ([product-maturity-audit-2026-07-26.md](../maturity-reports/product-maturity-audit-2026-07-26.md)): these are the international artifacts the audit found missing. Standard editions and numbers verified against sources on 2026-07-26, not cited from memory.
- **Closure note (2026-08-01):** The current open-board denominator is priced: `report-debt-economics` reports 19/19 open rows with principal + interest, all marked `Basis: estimate`. `08-validate-tracking` now treats that report as a hard invariant and also calls the forward-only new-row guard. The ATDM pass remains honest rather than decorative: `report-atdm-principal` reports 0/19 derivable rows because no open row declares a rule linkage plus occurrence/effort inputs, so no row is labelled `atdm`.
- **Acceptance criteria:**
  - [x] Every OPEN row carries a principal and an interest, with the unit declared.
  - [x] The tracking guard rejects a new open row without them.
  - [x] The automatable subset is derived from ATDM rather than hand-estimated.
...
**Title:** A runner stage lists its workspace packages by hand, so a transitive dependency added anywhere in the graph ships an image that dies at startup

- **Purpose:** Make "does this image contain everything it will require" a computed fact checked in CI, instead of a hand-maintained list that is correct until somebody adds an import three packages away.
- **Evidence:** **Every deployable image is a two-stage monorepo build whose runner copies `node_modules` plus a HAND-WRITTEN list of `<pkg>/dist` + `<pkg>/package.json` pairs, and `node_modules/@beyondnet/evolith-*` are workspace SYMLINKS into `src/packages/*`.** Copying `node_modules` therefore copies the links, not the packages: a link whose target was left off the list dangles, and Node fails at `require` time, inside the container, at startup. The image builds green and the push is green. **Reproduced locally on Docker 29.4.3 at the first attempt:** `EVOLITH_API_KEY=ci-load-key docker compose -f docker-compose.evolith.yml up -d --build redis core-api` produces a crash-looping `evolith-core-api`, and `docker logs` shows `Error: Cannot find module '@beyondnet/evolith-contracts/ingest'` with require stack `infra-providers/dist/tracker/evaluation-ingest.client.js` → `infra-providers/dist/index.js` → `core-api/dist/core-domain.module.js` → `core-api/dist/app.module.js` → `main.js`. Verified inside the image: `docker run --rm --entrypoint sh evolith-core-api:local -c "ls /repo/src/packages/contracts/dist/"` returns `No such file or directory`. **The `exports` map is not the defect** — `"./ingest"` is declared and maps to `./dist/ingest/evaluation-ingest.js`; the files were never copied. **The trigger is GT-604 (#314)**, which added `src/packages/infra-providers/src/tracker/evaluation-ingest.client.ts`; it imports `@beyondnet/evolith-contracts/ingest` for its VALUES and is re-exported from `infra-providers/dist/index.js`, so `contracts` joined the runtime require graph of every consumer for the first time. **Nobody forgot a direct dependency**: `core-api` declares `core-domain` and `infra-providers` and both were copied, and at that depth the list was complete — which is why review could not catch it. **Three images were affected, and the third was found by the guard rather than by reading:** `src/apps/core-api/Dockerfile`, `src/packages/mcp-server/Dockerfile` and `src/sdk/cli/Dockerfile` all omitted `contracts`; `src/apps/agent-runtime-api/Dockerfile` is genuinely complete (its closure is `agent-runtime` + `core-domain`). **Blast radius, and why it was mute:** the `Chaos drill` job of `.github/workflows/reliability.yml` failed on `develop` three consecutive times — runs `30628493705`, `30630994704`, `30632498438` — each at "Wait for the stack to serve a real verdict" with `[wait-for-target] TIMEOUT after 180s (last /health code=000)`. **The job captured no container logs at all**, so the exact require stack the container printed on every one of those runs was thrown away, and the only signal was a timeout that cannot distinguish a crash-loop from a slow boot. **The workflow was additionally blind to its own cause:** its `paths` filter covered `src/apps/core-api/**` and `src/packages/core-domain/**` but neither `infra-providers` nor `contracts`, so it never ran on the commit that broke it and turned red later on an unrelated core-api edit, pointing at the wrong change. **A cosmetic defect in the message compounded it:** `wait-for-target.sh` ran `curl -s -o /dev/null -w '%{http_code}' … || echo 000`, and since curl already prints `000` on a failed connection AND exits non-zero, the fallback CONCATENATED — the line reported `code=000000`. Confirmed by running the exact expression against a dead port. **Same failure class as [`GT-625`](./gap-reference-catalog.md#gt-625)**: a subpath import that resolves through the npm-workspace symlink at build time and breaks in a real install.
- **Component:** `Infra` · **Criticality:** P0 · **Complexity:** S
- **Provenance:** Diagnosed and fixed on 2026-07-31 from three consecutive red `Chaos drill` runs on `develop`. The root cause was confirmed by booting the image and listing the missing directory inside it, not inferred from the diff. The CLI Dockerfile was identified as a third victim by the guard written for this row, after the briefing had named only two.
- **Acceptance criteria:**
  - [x] `evolith-core-api:local` boots: the container reports `healthy` and `docker logs` contains no `MODULE_NOT_FOUND`.
  - [x] `bash product/infra/load/wait-for-target.sh` exits 0 against the rebuilt stack, on a real governed verdict rather than a `/health` 200.
  - [x] Every Dockerfile with a runner stage ships its full TRANSITIVE workspace closure — checked by computation, not by review. `56-validate-docker-workspace-closure` reports 4 images, closures of 3/4/6/7.
  - [x] The guard was run against the UNFIXED tree and turned red, naming all three Dockerfiles and both missing paths each — a guard nobody has watched fail is decoration.
  - [x] The guard ships negative fixtures that turn it red, including the transitive case itself, and green fixtures for the two false positives a naive version would raise (a type-only import, and a final stage not named `runner`). 17 tests.
...
- **Acceptance criteria:**
  - [x] A gap being worked is discoverable from the board together with the branch or PR that claims it, before the work lands. **Delivered 2026-08-01:** the board now points readers to `50-validate-gap-claim --claims-markdown`, a live view generated from open pull requests and their diffs. Its output lists each claimed gap with the pull request, branch, claim source (`declared`, `diff`, or both), and contested/claimed status; the view is intentionally generated at read time rather than committed as stale board state.
  - [x] A check fails at pull-request time when one `GT-*` id is claimed by more than one open branch, naming both claimants — `50-validate-gap-claim`, wired into `Governance guards (GT-578)`.
  - [x] The claim is DERIVED from open pull requests rather than hand-written, so it cannot be stale in the direction that matters — title, body and branch name of every open PR.
  - [x] It ships with negative fixtures that have been OBSERVED red — 10 of them, including two open PRs on one gap and the anti-vacuous floor, and `43-validate-guard-negative-fixtures` counts it at 39/39.
...
- **Acceptance criteria:**
  - [x] Registering a row on a branch cannot silently reuse an id that already exists on the base branch — `49-validate-gap-id-allocation` compares titles per id against the base and fails on a mismatch.
  - [x] The check runs on pull requests and names the colliding id together with both places it is used, so the resolution is not a search — wired into `Governance guards (GT-578)`, with the base fetched explicitly because a shallow checkout does not have it.
  - [x] It ships with negative fixtures that have been OBSERVED red — 13 of them, including the GT-634 collision reproduced over real git history and three anti-vacuous floors, and `43-validate-guard-negative-fixtures` counts it at 38/38.
...
- **Acceptance criteria:**
  - [x] `Governance guards (GT-578)` passes on `main` without the budget being raised to accommodate references that are already dead — it was LOWERED, 304 → 40, after the corpus was repaired.
  - [x] Each of the four references added since `4a1b92d6` is either repaired or named in the commit that accepts it, with the reason — subsumed: 269 of the 309 were repaired wholesale by the path migration.
  - [x] The guard reports the ratchet basis on a clean checkout, so the budget can never again be calibrated against a machine that happens to have built something.
...
- **Acceptance criteria:**
  - [x] `npm view @beyondnet/evolith-cli@latest dependencies` shows an SDK range that resolves to a version published on or after 2026-07-27, likewise `evolith-mcp` — both publish `^2.0.0` at 1.2.2, and a clean install resolves `sdk@2.0.0` with the binary booting.
  - [x] The fixes claimed by the 2026-07-23 security wave are verified present in the SDK version the CLI and MCP actually resolve, rather than assumed from the version number — and the verification REFUTED the premise: none of the seven wave commits touches `src/packages/sdk-client`, so there are no wave fixes in the SDK to be present or absent.
  - [x] `@beyondnet/evolith-sdk@1.1.0` is deprecated once a successor its consumers can reach exists — done 2026-07-30, with a message that states the real reason (superseded by 2.0.0, `.passed` → `verdict`) and says explicitly it is not a security advisory.
...
- **Purpose:** Remove two failure classes that generalise well beyond this file — a guard that reads its expectation from the artifact it is guarding, and a generator that stamps a hand-maintained field onto everything downstream of it.
- **Evidence:** **The guard compared the snapshot against six numbers copied out of the snapshot, so it could only ever pass — and the file it was failing to guard is stamped onto 388 rows of a larger artifact.** `native-evaluability-snapshot.json` declared in its own header that it was "a CAPTURED SNAPSHOT, not the source of truth". **No capture script existed.** It was hand-maintained and it drifted: it pinned `documentation-only: 129` while Core pinned 136, and went on calling rules `unimplemented-native` after they had handlers. Its guard in `iso-5055-mapping.test.mjs` asserted the snapshot's six class counts against six numbers typed into the test — the same six literals the snapshot already contained. **Expected value and actual value were copies of each other**, so the assertion held for as long as the file was unchanged, whatever Core actually pinned; it reported an agreement it never checked, which is worse than no guard, because the row it protects reads as verified. **The drift does not stay where it starts.** `build-iso-5055-mapping.mjs` stamps `nativeEvaluability` onto EVERY row of the ISO/IEC 5055 mapping from that file — 388 rows after the recapture, 381 before — so one stale class is laundered into a derived artifact five times the size of its input, and overstates the handler backlog that is [`GT-598`](./gap-reference-catalog.md#gt-598)'s entire deliverable by however many rules Core has since closed. The order that makes the difference (recapture, THEN rebuild) was written down nowhere, and the mapping's own guard ran in **no workflow at all**. **Fixed on 2026-07-29** (branch `claude/fervent-cohen-08ea01`): `capture-native-evaluability-snapshot.mjs` runs Core's REAL triage through `ts-node` rather than reimplementing the classification — a second implementation here would be a second source of truth, which is the defect being removed — and to make the triage reachable outside jest it moved from the spec into `src/packages/core-domain/test/rule-corpus-triage.ts`; that unreachability is precisely why the snapshot was maintained by hand in the first place. Three checks replace the tautology, each in the job that can afford it: `rule-corpus-triage.spec.ts` asserts the committed snapshot against a FRESHLY COMPUTED triage in both directions; the documentation-job guard, which has no `node_modules`, reads Core's pinned counts OUT of the spec that pins them and **throws** when the literal is missing or reshaped, so failing to find the source of truth can never read as agreement with it; and `--check` is the end-to-end re-derivation. **Recaptured on 2026-07-29 after merging the fix into the current line, and the drift had grown while it was in flight:** `native-handler` 139 → 151, `documentation-only` 129 → 136, the handler backlog 60 → **48**, corpus 379 → 386, mapping 381 → 388 rows. Twelve of those handlers were closed by [`GT-595`](./gap-reference-catalog.md#gt-595) and [`GT-632`](./gap-reference-catalog.md#gt-632) while the hand-maintained snapshot went on reporting them as backlog — the defect demonstrating itself one more time. The merge also carried a second instance of the pattern GT-595 recorded: the extracted `test/rule-corpus-triage.ts` had dropped the `enforce` field, so the measuring instrument would again have measured a different corpus than production evaluates; it is restored. Both steps are now links in the [`GT-630`](./gap-reference-catalog.md#gt-630) derived-artifact chain, which enforces the recapture→rebuild order as data instead of prose. **How it actually landed, recorded because three sessions converged on it:** the capture script and the extracted triage reached `main` as PR #276, the two chain links as PR #287, and a SECOND, independent implementation of the same fix reached `develop` as PR #279 — an in-spec renderer driven by `UPDATE_EVALUABILITY_SNAPSHOT=1` rather than a standalone script. Both recaptured the same numbers, which is mutual corroboration of the measurement; but two generators for one artifact is the defect of this very row reproduced one level up, and reconciling them is open work that blocks `develop` → `main`.
- **Component:** `Governance` · **Criticality:** P1 · **Complexity:** S
- **Provenance:** Follow-on defect from [`GT-598`](./gap-reference-catalog.md#gt-598), found on 2026-07-29 while extending its mapping: the snapshot GT-598 built on was never captured by anything, and the guard over it was tautological. Registered separately because the finding is a guard-design class, not a correction to GT-598's mapping.
- **Acceptance criteria:**
  - [x] The snapshot is produced by a script that runs Core's own triage instead of by hand, and `capture-native-evaluability-snapshot.mjs --check` re-derives it end to end.
  - [x] No guard over the snapshot takes its expected value from the snapshot: one recomputes the triage, the other reads Core's counts out of the spec that pins them and fails loudly when it cannot find them.
  - [x] The recapture→rebuild order is enforced as data — both links are declared in the `46-validate-derived-artifact-order` chain — and the mapping guard runs in a workflow, having run in none.
#### GT-636

**Title:** Three CI checks on PR #275 were red — all three were real, fixable defects, not accepted debt

- **Purpose:** Distinguish, with evidence rather than assumption, which parts of a PR's red CI are pre-existing branch debt versus a regression the PR introduced — and fix what a targeted investigation shows is actually fixable, rather than assuming red CI is unavoidable.
- **Evidence:** **Three CI checks were red on PR #275 — `Security Audit`, `E2E Tests`, `Evolith Core Validation` — and all three turned out to be real, fixable defects, not the accepted debt any of them first looked like.** `Evolith Core Validation` failed at `Bilingual Terminology Lint`: of 5 flagged terms, 2 were genuine untranslated prose (`database` in `0124-credential-secret-management-standard.es.md`, `ports and adapters` in `level-3-components/README.es.md`) and 2 were table/citation cells (`Portability` inside a clause citing ISO/IEC 25010:2023's own English characteristic-rename vocabulary; `Modular Monolith` / `Microservices` as 2 of 8 rows in a canonical topology-name table) where translating ONLY the flagged term while every sibling stayed in English would have made the document LESS consistent. `E2E Tests` failed on exactly 1 of 132 tests — `should validate architecture with --arch flag` called `validate --architecture --arch-level F1`, and neither flag ever existed in the CLI; the real flag is `-a, --arch` (`validate --help`), and Commander rejected the fake one as unknown, exiting `1`. `Security Audit` reported 31 `npm audit` findings (29 high, 2 moderate); deeper investigation traced 29 of them to a SINGLE root cause rather than 29 independent problems — a `brace-expansion` denial-of-service advisory (`GHSA-mh99-v99m-4gvg`) affecting every published version `<=5.0.7` and patched only in `5.0.8` (released after this repo's existing `overrides.brace-expansion: "2.1.2"` entry was set, which is why that override was ALREADY insufficient against the current advisory before this investigation began), propagating "high" severity up through the entire jest/eslint/`@nestjs/cli` devDependency toolchain via `via:` chains — plus one unrelated `fast-uri` finding.
- **Component:** `Cross` · **Criticality:** P2 · **Complexity:** S
- **Provenance:** Found on 2026-07-29 while confirming PR #275's CI was green after the `GT-637` ratchet fix; fixed the same day across three follow-up requests from the user ("arréglalos los tres" for the first two, then "arregla el security audit también" after this closure's own first pass had deliberately left it open as too risky to force blind).
- **Acceptance criteria:**
  - [x] `Evolith Core Validation`: the 5 flagged terms are corrected in place, or the lint's suggestion is overridden with a documented reason per term.
  - [x] `E2E Tests`: the `--arch` flag path that returned exit code `1` instead of `0`/`2` is root-caused and corrected.
  - [x] `Security Audit`: the 31 `npm audit` findings are triaged — upgraded where a non-breaking fix exists, or explicitly accepted with a documented reason where it does not.
  - [x] All three re-verified green with the exact CI command, not merely reproduced ad hoc.

**Closure evidence:** `Evolith Core Validation` and `E2E Tests` closed as described above (`database`→`base de datos`, `ports and adapters`→`puertos y adaptadores` translated directly; the other 3 marked with backticks as cited identifiers, which the linter's own header comment says it strips before scanning; the CLI flag corrected to `-a, --arch`; both re-verified with `bilingual-terminology-lint.mjs --verbose` and `npm run test:e2e` respectively). `Security Audit`: root `overrides.brace-expansion` bumped `2.1.2` → `5.0.8`, plus a new `overrides.fast-uri: "3.1.4"` entry (in-range of `ajv`'s own declared `^3.0.1`, confirmed via `npm view fast-uri versions`) — a 2-line `package.json` diff, net -149 lines in `package-lock.json` (fewer distinct duplicate sub-versions once properly deduped). Getting there required real forensics beyond editing the number: a plain `npm install` left 4 of the 10 tree positions (`eslint`, `@eslint/config-array`, `@eslint/eslintrc`, `fork-ts-checker-webpack-plugin`, each via their own un-hoisted `minimatch`) pinned to a STALE lockfile-recorded version regardless of the new override — the specific `packages` entries had to be deleted from `package-lock.json` before a fresh `npm install` would re-resolve that position at all, and this took TWO rounds (the first re-resolution landed on an intermediate `1.1.17`, itself still vulnerable, requiring a second delete-and-reinstall pass). A more targeted attempt — SCOPING the override under each affected parent (`"eslint": {"brace-expansion": "5.0.8"}`, matching this repo's own existing pattern for `eslint.ajv`) — made things WORSE, not better: once a package has ANY nested override object (even for an unrelated dependency like `ajv`), npm stops the plain top-level unscoped rule from cascading into that package's subtree at all, so the scoped attempt silently blocked the fix instead of applying it, and the working solution is the plain top-level override alone, with no nested entry competing for the same package name. 0 high / 0 critical confirmed with the EXACT CI command (`npm audit --audit-level=high`, real exit code 0 verified separately from the human-readable summary line, which was observed printing a stale count once) in an isolated, clean environment — a scratch `git worktree add --detach`, with a REAL, non-symlinked `node_modules` copied from the main checkout, since this session's own earlier per-package symlink farm (built for testing `@beyondnet/*` workspace packages against local sources) was independently confirmed to corrupt `npm ls`'s dependency-graph display (though not, it turned out, `npm audit`'s severity count, which matched across both environments throughout). 2 moderate findings remain, deliberately not chased: a Windows-only path-traversal in `@hono/node-server`'s `serve-static` (`GHSA-frvp-7c67-39w9`), irrelevant to this repo's Linux CI/deployment (confirmed via the k8s/helm infra this session inspected for an unrelated reason), whose available fix requires bumping an EXACT-PINNED production dependency (`@modelcontextprotocol/sdk`, pinned `1.29.0` with no range in two workspaces' `package.json`) rather than a devDependency override — out of scope for a security-audit closure that should not also silently change a shipped runtime dependency's resolved version. Verified beyond the audit itself, all in the same clean environment, all after rebuilding every workspace dependency exactly as the CI job's own "Build workspace dependencies" step does (core-domain, agent-runtime, sdk, infra-providers, core, mcp): full `core-domain` suite 1442/1442, CLI unit suite 1436/1436, CLI E2E suite 132/132, `mcp-server` suite 433/437 (4 pre-existing skips, not failures) — all green against the fixed dependency tree, confirming the override doesn't silently break anything that already passed.

#### GT-637

**Title:** The GT-578 dead-reference ratchet was stuck at 305 by two bugs in the guard itself, not the evidence backlog

- **Purpose:** Make the ratchet measure what it claims to measure — the evidence corpus's own path debt — instead of a parsing defect in the tool measuring it, and migrate the confidently-renameable slice of that debt.
- **Evidence:** **The GT-578 dead-reference ratchet was stuck at 305 by two real bugs in the GUARD, not by the size of the evidence-record backlog it claimed to measure.** Fixing the PR that closed `GT-633` surfaced a `Governance guards (GT-578)` CI failure and, investigating it, an uncommitted, unfinished refactor already sitting in the working tree: `resolveCommand` called `npmScriptOperand(...)`, a function that was never defined, so the guard crashed with a `ReferenceError` on every invocation — even in plain report mode, before `--strict` or `--max-dead` were ever reached. Repairing that function dropped the count from 309 to 78, which exposed the second bug: the corpus uses `npm run --workspace <path> <script>` throughout (the `--workspace` flag BEFORE the script name — a form `npm` accepts), and the resolver's script-name search had no notion that `--workspace` consumes the next token, so it mistook the workspace path itself for the script name and reported it as an undeclared script. Both bugs were pre-existing in the corpus's relationship to the guard; they were only visible together because, before the first fix, the guard never got far enough to reach the second.
- **Component:** `.harness` · **Criticality:** P1 · **Complexity:** S
- **Provenance:** Found on 2026-07-29 while verifying the `GT-633` PR's CI, and while investigating, an uncommitted WIP already mid-refactor toward this same fix was found in the working tree (confirmed with the user before building on it, per the session's own record — not silently discarded and not silently claimed as pre-existing work).
- **Acceptance criteria:**
  - [x] The guard runs to completion in report mode without crashing.
  - [x] `npm run --workspace <path> <script>` resolves correctly, with a negative twin proving a truly undeclared script under that form is still caught.
  - [x] Every confidently-renameable pre-`src/`/`product/`-move path in `validationCommands` is migrated, verified against a real successor file on disk before being written — never guessed.
  - [x] A gitignored, CI-downloaded artifact (`.harness/bin/opa`, `dist/`) is reported `unchecked`, not `dead`, with a negative twin proving a genuinely missing (non-gitignored) binary is still caught.
  - [x] `--max-dead` is lowered to the exact count observed by THIS runner (not a locally-derived estimate — the workflow's own comment names the prior incident where that distinction mattered), proven exact by observing the guard fail one below it.
  - [x] The residual dead count is retired-artifact or prose-misparse debt, individually accounted for — never force-mapped to a plausible-looking target.

**Closure evidence:** Both guard bugs fixed. `npmScriptOperand` is now defined (`.harness/scripts/ci/41-validate-evidence-commands.mjs`), correctly skipping `--workspace <path>` / `-w <path>` wherever it appears relative to the script name. That alone dropped 309 → 52. The remaining pre-`src/`-refactor and pre-`product/`-move paths this job's own docstring already named (`sdk/cli`, `apps/core-api`, `packages/*`, `reference/infrastructure/`, `reference/operations/`, the retired `@evolith/*` scope) were a DATA migration, not a code fix, and were already partially applied in the uncommitted state found in the tree; the rest were completed the same way — 18 further substitutions in `gap-closure-evidence.json`, each verified against a real successor file confirmed present on disk (`product/infra/docker-compose.yml`, `product/operations/**`, `.harness/scripts/ci/agentic/review-*.test.mjs`, `src/tests/contract/roundtrip-gate-evaluate.mjs`, `reference/core/control-center/gaps/gap-reference-catalog{,.es}.md`) before being written. That took 52 → 22. One more, `chat.command.ts` (a real `wc -l` in a GT-619 record), was missing only its directory prefix and was confirmed to exist at `src/sdk/cli/src/commands/chat/chat.command.ts`, closing 22 → 21. A third, unrelated defect was found and fixed on the way: `.harness/bin/opa` (15 of the original 309, the single largest bucket) is gitignored and downloaded by a separate CI step that never runs in this guard's job — the same finding as `GT-632` on guard 47, a gitignored artifact absent on a clean checkout mistaken for a wrong path, though the fix here is narrower: guard 47 could confirm a path's ANCESTOR was real and treat that as sufficient; this guard cannot confirm the leaf exists at all, so the honest verdict added is `unchecked`, not `resolved` — a `GITIGNORED_PREFIXES` check (`.harness/bin/`, `dist/` at any depth) applied to both the relative-binary and `node <script>` resolution branches. `.github/workflows/ci-cd.yml`'s ratchet step moved `--max-dead` from 305 to **21**, proven exact rather than padded: the guard passes at 21 and fails at 20. The residual 21 is genuinely retired or narrative, individually confirmed, not a rename: a consolidated `.bmad-core/AGENTS.md` that replaced per-agent files (`GT-232`, 2 records — different cardinality than a path move, not attempted), the retired `evolith-bff` gateway (`GT-270`–`272`, 3 records — a different service than the current `evolith-core-api`, not the same thing renamed), the retired `packages/mcp-tools` package (`GT-335`, `GT-352`, 2 records — no successor directory anywhere in the tree), two retired k8s manifests whose nearest-name successors have entirely different content rather than a moved path (`GT-462`, `GT-465`, 3 records — `broker-rbac.yaml`'s own text says "renamed from tenant-topology.yaml" and the board's own `GT-462` closure record already carries `dependencyDisposition: accepted-scope` for the retired message-path CRDs this manifest belonged to), a retired sandbox app (`apps/agent-sandbox/index.js`, 1 record, no successor found), three root-level `npm test` / `npm run info` / `npm run test:cov` invocations from before workspaces existed with no root-level successor script declared today (7 records), and three records (`GT-553`, `GT-554`, `GT-581`) where `vision/` or `tools/list` — an MCP JSON-RPC method name, not a filesystem path — appear inside PROSE describing history ("the only remaining mention of `vision/` is an explanatory comment", "was repointed from the deleted... to...") and are misparsed as path-shaped tokens by the same extractor ambiguity the guard's own docstring already names as its third command category. None of these 21 were force-mapped to a plausible-looking target on the theory that *something* had to close the count; a wrong rename would corrupt the record it claims to verify, which is worse than an honest residual. **Test coverage added, not just measured:** 5 new cases in `41-validate-evidence-commands.test.mjs` — the `--workspace`-before-script-name fix, its negative twin (an undeclared script under that form is still caught), the gitignored-artifact classification for both the relative-binary and `node <script>` branches, and its negative twin (a genuinely missing, non-gitignored binary is still `dead`) — so the fix cannot blind the guard the way the suite's own stated purpose says it must not. 39/39 self-tests pass. Verified beyond the guard itself: `08-validate-tracking` (633 gaps EN/ES, 610/610 catalog sections, 576 closure records), guards `42`–`45` in the same CI job, and a full `--execute --verbose` run of the guard in its actual report mode — 6 executed commands fail, all pre-existing and none among the paths touched here.

#### GT-633

**Title:** `evolith init` scaffolds a repository that cannot pass its own governance on the first run

- **Purpose:** Make the product's own bootstrap command produce a repository the product itself accepts — and, where a rule genuinely does not address a greenfield scaffold, say so in the applicability annotations instead of forcing the scaffold to fabricate an answer.
- **Evidence:** **`evolith init` produced a repository that fails its own first `evolith validate`, and only the GT-595 handlers made it visible.** With `MTN-05` and `GIT-08` given real handlers by the config-shaped slice, a freshly scaffolded satellite reported two blocking findings that were REAL verdicts rather than the GT-595 blocking-and-skipped invariant: the scaffold declared no `spec.boundedContexts`, and it wrote no commitlint configuration at all. The second is the sharper one — the corpus MANDATES Conventional Commits (`GIT-08`, `MUST`, `blocking`) and the product's own bootstrap command shipped a repository where nothing enforced them. Both were pinned by name in `rule-applicability.integration.spec.ts` (`expect(realViolations.map(i => i.ruleId).sort()).toEqual(['GIT-08', 'MTN-05'])`) precisely so that closing them would move that list to `[]`.
- **Component:** `Core Domain` · **Criticality:** P1 · **Complexity:** S
- **Provenance:** Found on 2026-07-29 as the direct follow-on to `GT-595`, which closed both rules for real and pinned the two failures it exposed rather than filtering them out.
- **Acceptance criteria:**
  - [x] A freshly initialized satellite reports NO blocking finding that is a real verdict — every remaining one is the `GT-595` blocking-and-skipped invariant, and the pinned array reads `[]`.
  - [x] `GIT-08` passes because the scaffold emits all three legs its handler reads — configuration, declared package, and something that runs it — not because the rule stopped applying.
  - [x] `MTN-05` is DEFERRED rather than disabled: excluded at phases 0 and 1, still binding from Design (phase 2) onward, asserted by test.
  - [x] Each rule verified individually against a freshly built `dist`, not through the suite alone.

**Closure evidence:** The two halves were closed by OPPOSITE means, because they are not the same kind of defect. **`GIT-08` was a scaffold gap and nothing else.** Conventional Commits bind from the first commit and the corpus already states the convention and its seven types verbatim, so there was nothing to decide and nothing to invent. `ProjectScaffolderService.scaffoldCommitConventions` now emits `commitlint.config.mjs`, merges `@commitlint/cli` and `@commitlint/config-conventional` into `devDependencies`, and — with `--features hooks` — writes a `.husky/commit-msg` running `npx --no-install commitlint --edit "$1"`. The `--no-install` is the whole point: it makes a missing commitlint an ERROR rather than the silent skip `GT-623` found, where a hook's `else` branch printed "commitlint is not installed — skipping" and exited zero. When `hooks` is not selected the config is still written and the result carries a warning saying nothing runs it, rather than leaving a file that looks like enforcement and is not. It is runtime-independent on purpose: commitlint is a Node tool, so a .NET or Python satellite receives a tooling-only `package.json` — what such repositories carry in practice, and what the handler actually reads; emitting the config alone would have left the rule failing for every non-Node runtime. **`MTN-05` was NOT a scaffold gap, and emitting a `spec.boundedContexts` stanza would have been the wrong fix.** The rule's own description says the strategy MUST be defined "before Phase 2 Design", and a phase-0 scaffold has no bounded contexts to declare; satisfying it at `init` would have meant writing an INVENTED persistence decision into every new repository, after which `MTN-05` would pass for a reason nobody chose. Worse, per the handler's own note the two vocabularies are irreconcilable — `evolith-yaml.schema.json` restricts `persistence` to a database-engine enum (`PostgreSQL | MongoDB | SQLServer | SQLite`) under `additionalProperties: false`, while `MTN-05` names three multi-tenancy STRATEGIES — so any value written either breaks the contract schema or cannot match the rule as worded. It carries `appliesFromSdlcPhase: 2` instead, which is the applicability fact the rule always stated in prose, and follows the `TAX-06` precedent (a phase annotation alongside a parallel rego). The deferral is asserted, not assumed: a test walks phases 0→3 and requires the rule to be excluded at 0 and 1 and to bind again at 2 and 3, so a future weakening of the annotation turns it red. **A methodological finding worth more than the fix.** The premise did not reproduce on first contact. The suite resolves `@beyondnet/evolith-core-domain` through the npm-workspace symlink to `dist/`, which was nine days stale and predated the GT-595 handlers, so both rules were reported `skipped` and the pinned assertion ALREADY read `[]`. A green run in that state would have "proved" the gap closed before a line was written — the same class as `GT-625` and `GT-632`, a pass that depends on untracked build state, and here it would have produced a false CLOSURE rather than a false failure. It was reproduced only after re-anchoring `node_modules` at the worktree (the shared tree's links point at the main checkout, so a worktree silently tests somebody else's sources) and rebuilding. Each rule was then verified individually rather than through the suite: `GIT-08` executes and PASSES for both the typescript and python scaffolds; `MTN-05` appears in `notApplicableRuleIds` rather than passing silently. Verified: core-domain 1442/1442, infra-providers 129/129, sdk/cli 1436/1436, `32-validate-ruleset-schemas` 168/168, `28-native-evaluator-parity` 451 assertions, `01-validate-docs` 1491 files. **Not done and deliberately so:** the parallel `multi-tenancy.rego` still asserts `MTN-05` unconditionally. Applicability is a native-engine pre-filter by construction (`GT-571`), and `TAX-06` already carries the same shape, so this is a pre-existing engine divergence rather than one introduced here.
...
- **Acceptance criteria:**
  ...
  - [x] A guard catches a JOINED path that does not exist, not only a literal one — the gap `40-validate-path-literals` cannot see.

**Criterion 3 CLOSED 2026-07-29 — and the guard's first run was mostly wrong, which is the finding.** `47-validate-joined-paths` resolves every `path.join(<repo-root base>, 'a', 'b')` whose segments are literals and requires the target to exist. It reported **19 broken paths; only 3 were defects.** Each of the other 16 would have been made WORSE by "fixing" it, and the three reasons are now modelled rather than waved away. Six had base `root`, which in the evaluators is a PARAMETER naming the customer workspace under evaluation — `path.join(root, 'agent.config.json')` asks about THEIR repository, and correcting it would have broken evaluation for every satellite. Nine were members of FALLBACK CHAINS: `pattern-catalog` and `topology-catalog` each try four layouts and are correct when one resolves; demanding all four would require every historical layout simultaneously, and the natural way to silence that is to delete the fallbacks, breaking already-deployed images. One was a PROHIBITION: `evalNoRootTopologies` FAILS when `topologies/` exists, so its absence is the rule being satisfied and correcting the path would have inverted the rule. Chains are now detected structurally (siblings in an array literal, or the two arms of a ternary); prohibitions cannot be — no syntax distinguishes "must exist" from "must not" — so they are declared with a written reason. **The three real defects:** `opa-evaluator.ts` resolved both `policy.wasm` and the input schemas without the `src/` prefix, the same class as the P0 above and equally fail-closed; `satellite-upgrade-diff.ts` probed `rulesets` behind an early return, so a satellite upgrade reported zero ruleset changes, indistinguishable from having none; and `artifact-path-resolver.ts` was the lone straggler among twenty siblings, naming an `adr-matrix.json` that exists in no layout. Ships with 11 negative fixtures covering both anti-vacuous floors, and `43-validate-guard-negative-fixtures` OBSERVED it turning red (36 of 36 guards) rather than accepting a declaration. A known weakness is recorded in its header instead of hidden: two joins as sibling function ARGUMENTS are scored leniently as a chain, which is the conservative direction for a guard whose false positives push people to break working code. core-domain 1367/1367, mcp-server 434/434, core-api 179/179, CLI 1436/1436.
...
- **Acceptance criteria:**
  ...
  - [x] A document that violates the schema it itself declares still WARNs, and a standard-shaped ruleset that fails the standard schema still WARNs. Both directions are pinned by tests, so silencing did not become blindness.
  - [x] All three documents now satisfy their own declared schemas: `rule-definition.schema.json` models the `validation` block the two INFRA files always carried, the files use `title`/enum severities, and their `$schema` and target paths point at trees that exist (`product/infra/…`).
  - [x] The real corpus loads with **zero** warnings and **zero** errors — asserted against the actual repository tree, not a fixture, because a fixture cannot tell us the corpus as authored is clean.
  - [x] The five `phase-f*.json` files are restored, conform to `sdlc-phase.schema.json` (`order` renumbered 1–5 to satisfy its `minimum: 1`, `$schema` normalized to the `$id` form the gate files already use), and the five gate files are reachable again.
  - [x] `NestLoggerProvider` forwards `context` only when it exists; the one-argument and two-argument forms are both pinned by tests. Verified end-to-end: **44 → 0** `WARN undefined` lines over an identical eleven-request run.
  - [x] **The verdict change from fault (2) is understood and accepted by the board before this reaches a satellite.** Restoring the phase index is a governance-visible change, not a silent repair: a satellite that was passing because its F1 gate never ran can now legitimately FAIL. For the k6 smoke payload the top-level verdict is unchanged (`FAIL` before and after, already failing on general rulesets), but that payload does not exercise the flip and must not be read as evidence that no payload does. **Board acceptance recorded 2026-08-01 in this board-owned catalog:** the fail-closed phase-gate change is intentional and may surface newly legitimate satellite failures; k6 smoke is retained only as same-payload evidence, not as a universal verdict-invariance claim.
  - [x] The two unwired duplicates of the same logger adapter — `src/apps/core-api/src/infrastructure/providers/logger.provider.ts` and `src/sdk/cli/src/infrastructure/providers/logger.provider.ts` — are removed. Neither is reachable (both surfaces resolve `NestLoggerProvider` from `@beyondnet/evolith-infra-providers`), but both still carry fault (3) and their specs still assert it. **Verified 2026-08-01:** the duplicate files and specs are absent from both trees; current imports resolve only the shared `@beyondnet/evolith-infra-providers` `NestLoggerProvider`, whose one-argument and two-argument forms are pinned by tests.

#### GT-648

**Title:** The deployed Core re-reads and re-validates its whole ruleset corpus on every evaluate call, blocking the event loop per request

- **Purpose:** Make the ruleset corpus what it has always been — deployment state, read once — so a governed evaluation stops paying for a disk walk and a schema pass that cannot produce a different answer than the previous request's.
- **Evidence:** **`RuleEvaluationEngine.discoverAndEvaluate` (`rule-evaluation-engine.ts:308`) calls `rulesetRepo.loadAllRulesets(corePath)` on every evaluation, and core-api's `IRulesetRepository` provider is a bare `DiskRulesetRepository` with no cache** (`core-domain.module.ts`). Each call walks the corpus tree, reads 176 `*.rules.json` files, and runs each through Ajv to produce 393 normalized rules. Both the walk and the Ajv pass are synchronous CPU work inside `await`ed calls, so the Node event loop cannot advance while they run — which makes this a whole-process latency defect, not a slow endpoint. **Observed in CI run `30631939687` (Reliability workflow on `beyondnetcode/evolith`, artifacts `k6-smoke.json` and `core-api.log`):** the corpus's load-time WARN block — *"Phases directory not found …"*, plus three *"Skipping non-standard ruleset …"* lines — repeats once per k6 iteration at roughly 1/s. A boot-time load would emit it exactly once, so the log itself is the proof that the corpus is being loaded per request. **The measured consequence, from the same run:** a `GET /health` that landed during that work took **498.5 ms end-to-end** while core-api logged its own handler at `durationMs=0`, with `http_req_waiting` 498.4 ms against `http_req_connecting` 0.17 ms. Nothing about the health handler was slow; the request sat in the accept queue behind a loop that was not running callbacks. **Reproduced locally on 2026-07-31 against `origin/develop` at `3051e499`, and the reproduction is exact:** eleven evaluations produced eleven corpus loads — 33 *"Skipping non-standard ruleset"* lines at three per load, and 11 *"Phases directory not found"* lines at one per load. One `loadAllRulesets` call, timed directly on that corpus, cost **30.4–54.2 ms**, and a 10 ms timer queued across it fired up to **14 ms late** — the event-loop block, measured rather than inferred. **That laptop figure is roughly a tenth of the runner's 498 ms, and the discrepancy is not a weakness of the finding but its shape:** an SSD with a warm page cache is the best case for exactly this work, and the runner is the ordinary one. **Compounding it, and also per request:** `SdlcDataLoaderService.loadAllGates` re-reads the phase index once per phase, because `loadGatesForPhase` → `loadPhase` → `loadAllPhases`, turning one directory scan into N+1. **Found while fixing a flaky k6 threshold in `product/infra/load/k6/smoke.js`.** That fix — asserting the median instead of a 10-sample p95 — stops the flake gating CI and deliberately does not address this defect; it is *why* `/health` has a multi-hundred-ms tail at 1 VU.
- **Component:** `Core API` · **Criticality:** P1 · **Complexity:** S
- **Provenance:** Registered 2026-07-31 from CI run `30631939687` and reproduced locally the same day. Id assigned by union of `origin/main..origin/develop` and the open pull requests (`#325` and the dependabot set), whose maximum was `GT-645`. Split from [`GT-649`](./gap-reference-catalog.md#gt-649) deliberately: the WARNs in that log are a *symptom* of this per-request load but are independent corpus defects, and folding them together is the one-gap-one-claim failure [`GT-639`](./gap-reference-catalog.md#gt-639) exists to prevent.
- **Acceptance criteria:**
  - [x] The corpus is loaded and schema-validated **once** for a given `corePath`, and later evaluations read it from memory. Delivered as `CachingRulesetRepository`, a decorator over `IRulesetRepository` — the cache belongs to the long-running deployment, not to the disk adapter, because the CLI is a one-shot process that must keep re-reading disk between invocations.
  - [x] The load happens **before** the server accepts traffic, so request #1 does not pay it either. `RulesetCorpusWarmupService` (an `OnApplicationBootstrap` hook) preloads it and logs the count as evidence: `Ruleset corpus loaded at startup: 393 rules … (loaded once; evaluations no longer re-read disk)`.
  - [x] Invalidation is **explicit**, not time-based (`invalidate(corePath?)`). A TTL would only reintroduce the same stall on an unpredictable schedule; the corpus is baked into the image alongside the process, so there is no cadence to guess at.
  - [x] Concurrent cold loads collapse into one disk read (the in-flight *promise* is memoized), or the cache would merely move the stampede from steady state to burst time.
  - [x] Failures are never cached, so [`GT-474`](./gap-reference-catalog.md#gt-474) keeps its meaning: an unresolvable corpus still throws per request with its full probe trail, and a corpus repaired on disk loads on the next attempt instead of latching the process into permanent failure.
  - [x] **Verdicts are unchanged, verified rather than argued.** The full `EvaluationResult` for the k6 smoke payload was captured from a build of `origin/develop` and from this change with the [`GT-649`](./gap-reference-catalog.md#gt-649) phase data withheld, and the two are **byte-for-byte identical at 442,204 bytes**. Separately, the normalized corpus itself was diffed against the pre-change loader: **393 rules, identical JSON**.
  - [x] Measured after: the corpus loads once at boot (393 rules, 181–590 ms), evaluate median falls 52.7 → 38.2 ms, and a `/health` issued 5 ms into an in-flight evaluate falls **11.6 → 3.5 ms**. The per-request WARN block is gone from the log entirely.
  - [x] Re-measured on a CI runner rather than a laptop, against the `k6-smoke.json` of a later Reliability run, to confirm the 498 ms tail is gone where it was observed. **CLOSED 2026-08-01 on runner evidence:** Reliability run `30680099586`, job `k6 load profile (thresholds + published metrics)`, head `6fc31587`, published `k6-smoke.json`, `k6-average.json`, and `core-api.log`. `RulesetCorpusWarmupService` loaded the corpus once at startup — 393 rules in 75 ms — and the per-request WARN block is absent (`Skipping non-standard ruleset`, `Phases directory not found`, and `WARN undefined` all 0). The smoke artifact reports 20 requests / 50 checks / 0 failures, `health_latency` p99 **4.24 ms** and max **4.33 ms**, while `evaluate_latency` median is **64.1 ms** and p95 **66.9 ms**. The longer average profile confirms the same shape over 1,143 requests: `health_latency` p99 **4.87 ms**, max **7.11 ms**, and no recurrence of the 498 ms health tail where the defect was first observed.

#### GT-656

**Title:** A guard against id collisions made gap titles immutable

- **Purpose / Problem:** Let a row's headline be corrected when it is measured wrong, without weakening the check that stops two sessions from allocating one number to two different gaps.
- **Evidence:** `49-validate-gap-id-allocation` compares each catalog `**Title:**` against the base branch and fails on any difference. The title is the right discriminator — evidence, status and criteria all change on a branch, and only the title says WHICH gap the number names — but the guard treated a deliberate retitle and a collision as the same event, and had no channel for the difference. Its own failure text admitted this and asked for the distinction "in the commit"; nothing reads commit messages, and a squash merge rewrites them, so in practice the answer was always "renumber or leave it wrong".
- **What it means:** titles were effectively immutable. A board that exists to stop technical debt from being described inaccurately accumulated rows whose FIRST LINE was inaccurate, which is the line a reader sees.
- **The cost was already paid.** [`GT-622`](./gap-reference-catalog.md#gt-622) was re-measured twice — 82 → 201 → 210 orphaned analyses, and the branch still carrying the warning turned out to be `develop`, not `main` — while its headline went on reading "Eighty-two ... every PR". Its evidence field was corrected on 2026-08-01 and again on 2026-08-08; the title was not, both times because correcting it turns a REQUIRED check red on the very PR carrying the correction. The first occurrence set the precedent silently; the second found the precedent and was about to follow it.
- **Component:** `.harness` · **Criticality:** P2 · **Complexity:** S
- **Principal:** `S` · **Interest:** `MED` · **Basis:** `estimate`
- **Provenance:** Found on 2026-08-08 while closing [`GT-622`](./gap-reference-catalog.md#gt-622), when correcting that row's measurably wrong title turned `Validate documentation` red. Registered rather than worked around: leaving the title wrong a third time would have made the workaround the convention.
- **Acceptance criteria:**
  - [x] A deliberate retitle can land without weakening the collision check — declared in `reference/core/control-center/gaps/gap-retitles.json` as `{ id, from, to, declaredAt, reason }`, and exempting only when BOTH titles match the two branches exactly.
  - [x] The exemption is scoped to a retitle, never to an id. Covered by `classifyRetitles` and by the fixture *"a real collision landing on an already-retitled id still fails"*: once the declared titles are not the two on the branches, the collision is reported again.
  - [x] A declaration cannot rot into a silent pass. `active` exempts, `spent` is reported (the retitle reached the base, so nothing is left to exempt), `rot` — describing neither side — FAILS, as does a registry that exists but cannot be parsed or has no `retitles` array.
  - [x] Every exemption is printed, never applied silently: `declared retitles ... N active, N spent, N rot` plus one line per active declaration carrying its reason.
  - [x] The negative twin is observed red, not assumed: the same retitle WITHOUT a declaration still fails, and a declaration quoting a title the catalog never carried does not launder it. `node --test` 27/27.
- **First use:** GT-622's title is corrected in the same change that built the mechanism — 82 → 210, "every PR" → "PRs into `develop`" — with the declaration carrying why. That is the closure's own proof: the guard is green with the retitle applied against both `origin/main` and `origin/develop`.
- **Status:** `DONE` (2026-08-08)

#### GT-657

**Title:** A js-yaml advisory with no upstream fix, and an audit job with no way to say so

- **Purpose / Problem:** Clear the half of a new HIGH advisory that a version pin can clear, and give the audit job a way to express the other half — an advisory this repository provably cannot fix — without leaving a check permanently red.
- **Evidence:** `GHSA-5p4m-2wfm-xmqj` / CVE-2026-59870 (js-yaml, quadratic CPU consumption in `!!omap` resolution) was published between 2026-08-05 and 2026-08-08. The dating is evidence, not colour: the dependabot PRs of the 5th were green, PR #440 of the 8th was red, and #440 changed 8 documentation and JSON files with **zero** dependency files. So it is pre-existing branch debt on `main` and `develop`, not a regression — the same distinction [`GT-636`](./gap-reference-catalog.md#gt-636) had to make.
- **The half that had a fix, and why it was missed:** the root `overrides` already carried `js-yaml: 4.3.0`, added for an EARLIER advisory. The new one is vulnerable through `4.0.0 - 4.3.0` inclusive, so the existing pin was exactly one patch short of it. `4.3.1` clears it, and re-resolving also collapsed three nested copies (`@commitlint/load`, `@istanbuljs/load-nyc-config`, `cosmiconfig`) into the single hoisted one.
- **The half that has none.** The remaining advisory arrives through `@nestjs/swagger`, which pins `js-yaml` **exactly**, and every published release pins a vulnerable version: `11.4.4` → `4.1.1` and `11.4.5` → `4.3.0` (both inside CVE-2026-59870), `11.4.6` → `5.2.1` (inside `GHSA-pm4m-ph32-ghv5`, vulnerable `5.0.0 - 5.2.1`). `11.4.6` is the latest stable; `12.0.0` exists only as alpha. There is no version to bump to.
- **npm `overrides` do not reach it, and that was measured rather than assumed** — four routes, same tree every time: a top-level `js-yaml` override; a scoped `@nestjs/swagger: { js-yaml }` override; both repeated with the unrelated nested override objects (`eslint`/`@eslint/eslintrc` → `ajv`) temporarily removed, to test whether their presence blocked the cascade; and `npm install --package-lock-only` versus a real `npm install`. **This REFUTES the generalisation [`GT-636`](./gap-reference-catalog.md#gt-636) recorded** — that a nested override object anywhere stops the top-level rule cascading. Removing them changed nothing; what actually blocks the override is the consumer's EXACT pin. Recorded here rather than left standing, because a wrong lesson in a closed row is worse than no lesson.
- **Why an acceptance was not enough.** Leaving `Security Audit` red is exactly what [`GT-622`](./gap-reference-catalog.md#gt-622) had just been closed to remove: a permanently red check trains reviewers to discount red checks, and the next genuinely fixable advisory would land in a job everyone had learned to ignore. The threshold is unchanged; what is added is that an unfixable advisory must be NAMED.
- **Component:** `Security` · **Criticality:** P2 · **Complexity:** M
- **Principal:** `M` · **Interest:** `HIGH` · **Basis:** `estimate`
- **Provenance:** Found on 2026-08-08 as the one red check left on PR #440 after GT-622 and GT-656 went green. Registered rather than dismissed as noise, because "not caused by this PR" is a statement about attribution, not about whether the advisory is real.
- **Acceptance criteria:**
  - [x] The fixable half is fixed at its source: root override `js-yaml` `4.3.0` → `4.3.1`, verified by re-resolution — one hoisted `4.3.1` where there were four positions, and CVE-2026-59870 gone from the report.
  - [x] The unfixable half is proven unfixable before being accepted: all three `@nestjs/swagger` releases enumerated with the exact `js-yaml` each pins, and four override routes measured.
  - [x] `Security Audit` fails on any high/critical advisory that is not declared — the threshold is unchanged, the fixtures observe it red.
  - [x] A declaration is scoped to an advisory AND the path it arrives by, so it cannot excuse the same id appearing somewhere else later.
  - [x] A declaration whose advisory has disappeared FAILS as stale, so the good news arrives as a red check asking for the entry's removal instead of an exemption nobody re-reads.
  - [x] The gate's own fixtures run in the job — a gate whose exception list can swallow anything is a green button. `node --test` 19/19.
