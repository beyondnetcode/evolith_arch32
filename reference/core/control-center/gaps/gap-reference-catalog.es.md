...
**Title:** Tres checks de CI en el PR #275 estaban en rojo — dos eran bugs reales encontrados al investigar, uno es deuda genuina de devDependencies dejada abierta a propósito

- **Purpose:** Distinguir, con evidencia en vez de suposición, qué parte del CI en rojo de un PR es deuda preexistente de la rama frente a una regresión que introdujo el PR — arreglar lo que una investigación dirigida muestra que es genuinamente arreglable, y dejar lo que genuinamente no lo es como una decisión acotada y documentada en vez de forzarla.
- **Evidence:** **Tres checks de CI estaban en rojo en el PR #275 — `Security Audit`, `E2E Tests`, `Evolith Core Validation` — y dos resultaron ser bugs reales que la investigación destapó, no la deuda aceptada que parecían al principio.** `Evolith Core Validation` fallaba en `Bilingual Terminology Lint`: de 5 términos marcados, 2 eran prosa genuinamente sin traducir (`database` en `0124-credential-secret-management-standard.es.md`, `ports and adapters` en `level-3-components/README.es.md`) y 2 eran celdas de tabla/cita (`Portability` dentro de una cláusula que cita el vocabulario propio en inglés de ISO/IEC 25010:2023 sobre el renombre de características, en la prosa de `GT-598` dentro de `gap-tracking.es.md`; `Modular Monolith` / `Microservices` como 2 de 8 filas en una tabla canónica de nombres de topología en `maturity-assessment.es.md`) donde traducir SOLO el término marcado mientras cada vecino seguía en inglés habría hecho el documento MENOS consistente, no más. `E2E Tests` fallaba en exactamente 1 de 132 tests — `should validate architecture with --arch flag` llamaba a `validate --architecture --arch-level F1`, y ninguno de los dos flags existió jamás en el CLI (`grep -rn "arch-level" src/sdk/cli/src/` no devuelve nada; `validate --help` nombra el flag real como `-a, --arch`, un booleano sin argumento de nivel). Commander rechazaba `--architecture` como desconocido y salía con `1`, que el propio contrato `[0, 2]` del test no puede distinguir de un crash genuino.
- **Component:** `Cross` · **Criticality:** P2 · **Complexity:** S
- **Provenance:** Encontrado el 2026-07-29 al confirmar que el CI del PR #275 quedaba verde tras el arreglo del ratchet de `GT-634`; arreglado el mismo día a pedido explícito de seguimiento del usuario ("arréglalos los tres"), tras el primer pase de solo-investigación.
- **Acceptance criteria:**
  ...
  - [ ] `Security Audit`: los 31 hallazgos de `npm audit` quedan triados — actualizados donde exista un arreglo no disruptivo, o aceptados explícitamente con motivo documentado donde no lo haya.
  - [x] Los dos checks arreglados reverificados en verde con el comando exacto de CI, no solo reproducidos de forma ad hoc.

**Evidencia de cierre (parcial — 2 de 3):** `Evolith Core Validation`: `database`→`base de datos` y `ports and adapters`→`puertos y adaptadores` traducidos directamente in situ. Las otras 3 instancias marcadas se dejan con backticks en vez de traducirse — `` `Portability` ``, `` `Usability` ``→`` `Interaction capability` `` (su vecino intacto en la misma cláusula), y `` `Modular Monolith` `` / `` `Microservices` `` en la tabla de topologías — que el propio comentario de cabecera de `bilingual-terminology-lint.mjs` dice que excluye del escaneo de prosa por ser identificadores de código; es una excepción documentada de "esto es un término/identificador citado, no narrativa" vía el mecanismo que la herramienta ya ofrece, no una elusión. Reverificado: `node .harness/scripts/bilingual-terminology-lint.mjs --verbose` pasa; `01-validate-docs` (1491 ficheros) y `04-check-bilingual-parity` sin afectar. `E2E Tests`: el flag corregido a `-a, --arch` en `test/e2e/cli-e2e.test.ts`, con un comentario que registra por qué `--architecture`/`--arch-level` nunca funcionaron. Reproducirlo en local exigió primero descubrir que `npx tsc -b tsconfig.json` (usado antes en esta sesión para builds no relacionados) se salta el hook de ciclo de vida `prebuild` de npm, así que `copy-rulesets` nunca corría y el `rulesets/` empaquetado del CLI solo tenía un fixture `agents/` obsoleto — todo intento manual de reproducción fallaba con un `RULESET_NOT_FOUND` no relacionado hasta usar `npm run build`, que sí dispara `prebuild` correctamente. Con el corpus real empaquetado, `validate --satellite <fixture> --arch` devuelve el código de salida `2` (veredicto bloqueado, 61 reglas evaluadas, 117 saltadas por aplicabilidad) — casando con el propio contrato `[0, 2]` del test. Suite completa reverificada con el comando exacto de CI: `npm run test:e2e` → 18 suites, 132/132 pasaron. `Security Audit`: investigado más a fondo, no arreglado, y encontrado MÁS PROFUNDO de lo que registró el primer pase. Aislado en un worktree de git de scratch (`git worktree add --detach`) con una copia REAL, no-symlinked, de `node_modules` copiada del checkout principal — la granja de symlinks por paquete que esta sesión armó antes para probar los paquetes de workspace `@beyondnet/*` contra fuentes locales quedó confirmada corrompiendo la propia visualización del grafo de dependencias de `npm ls` (fabricando aristas como "cross-spawn depende de jest"), aunque NO el conteo de severidad de `npm audit`, que coincidió en 31 tanto en el entorno corrompido como en el limpio. Se trazaron dos cadenas vulnerables reales, ambas arraigadas en una versión de `brace-expansion` que el `overrides.brace-expansion: "2.1.2"` YA EXISTENTE en la raíz del repo no alcanza: `babel-plugin-istanbul` (traído vía `ts-jest` → `@jest/transform`, a su vez vía `test-exclude@6.0.0`) y el propio `minimatch@3.1.5` empaquetado de `eslint@9.39.4`, cada uno resolviendo un `brace-expansion@1.1.16` ANIDADO y no-hoisteable en `node_modules/<padre>/node_modules/brace-expansion` en vez de usar el override de la raíz. Por separado, `fast-uri@3.1.2` (bajo `ajv@8.20.0`) es subible con seguridad a `3.1.4` — dentro del rango que el propio `ajv` declara (`^3.0.1`), confirmado con `npm view fast-uri versions`. El `npm audit fix` plano, no forzado, ni siquiera puede intentar nada de esto: falla de inmediato con un `ERESOLVE` de conflicto de peer no relacionado en `@typescript-eslint/parser`. Cada uno de los 31 paquetes marcados es una devDependency (el toolchain de build-y-test jest/eslint/`@nestjs/cli`/babel) sin exposición en runtime de producción — ninguno se embarca en los artefactos construidos del CLI, el servidor MCP o el Core API. Se deja abierto deliberadamente: el arreglo real es o un `overrides` nuevo, anidado y dirigido (siguiendo el propio patrón ya establecido en este repo de overrides con alcance, como `overrides.eslint.ajv`, pero sin verificar aquí contra la corrida real de lint/test de cada workspace) o un bump de versión mayor del tooling central en unos 10 workspaces — ambos son el tipo de cambio difícil de revertir y de radio de impacto amplio sobre un lockfile compartido de todo el monorepo que no debería forzarse unilateralmente al final de una sesión larga sin pruebas de regresión dedicadas contra cada consumidor.
