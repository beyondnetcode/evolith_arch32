...
| [`GT-406`](./gap-reference-catalog.md#gt-406) | **External HITL adapters:** Missing real Slack/Tracker/GitHub human approval ports. | `Agent Runtime` | Cross | P1 | M | `DEFERRED` |
...
| [`GT-387`](./gap-reference-catalog.md#gt-387) | R4 — HITL approval workflow: replace `AutoApprovalAdapter`/`DenyByDefaultApprovalAdapter` with a real chat/Tracker human-in-the-loop workflow behind `IApprovalPort` (production currently deny-by-default for high-impact capabilities). | `Agent Runtime` | Cross | P2 | M | `DEFERRED` |
...
| [`GT-394`](./gap-reference-catalog.md#gt-394) | Per-tenant ABAC on corpus access in core-api — prevent a tenant reading another tenant's rulesets/corpus (the stateless Core serves a shared corpus today). (Extracted from `EVOLITH-ARCHITECTURE-DESIGN.md` §16 risk.) | `Core API` | Cross | P2 | M | `DEFERRED` |
| [`GT-404`](./gap-reference-catalog.md#gt-404) | **OpenCode adapter:** Not implemented. OpenCode lacks secure runtime capability adapter. | `Agent Runtime` | Cross | P2 | M | `DEFERRED` |
...
**Progress:** 397 / 412 done · 8 in progress · 7 pending · 0 deferred
