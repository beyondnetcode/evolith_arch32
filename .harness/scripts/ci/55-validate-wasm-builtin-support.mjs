...
async function main() {
  ...
  // GT-675 — `evolith.manifest` has no `.rego` in the tree BY DESIGN.
  //
  // It is generated by `compile-opa-wasm.mjs` into a staging directory, compiled
  // into the bundle, and deleted. That is deliberate: it carries the set of rule
  // ids the bundle can decide, read from the OPA compiler's own AST, and a
  // COMMITTED copy could drift from the artifact that actually runs — which is
  // precisely the failure this guard exists to prevent, one level up.
  //
  // The exemption is verified, not asserted: the package must still appear as a
  // real entrypoint in the compiled wasm, so a build that stops producing it
  // fails here rather than quietly dropping the check.
  const GENERATED_PACKAGES = new Map([
    ['evolith.manifest', 'evolith/manifest/declared_rule_ids'],
  ]);
  for (const [pkg, entrypoint] of GENERATED_PACKAGES) {
    if (!entrypoints.includes(entrypoint)) continue;
    // Contributes no reachable policy source, so it cannot mask an empty scan.
