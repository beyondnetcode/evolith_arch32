...
export const GENERATED_DOC_EXCLUSIONS = [
  {
    ...
    // Was count-pinned (expectedFiles: 15) because the projector stamped nothing. Pinning an
    // inventory is the weaker proof: it cannot tell a hand-written file from a generated one
    // as long as the total happens to match. The projector now emits GENERATED_BANNER into
    // every document it writes — concepts carry it just after their OKF frontmatter, the
    // reserved index.md/log.md at the top — so membership is verified per file against
    // content, and an unstamped .md dropped into the tree stays in scope.
    marker: 'GENERATED by .harness/scripts/knowledge-okf-project.mjs',
  },
  {
    id: 'wiki-mirror',
