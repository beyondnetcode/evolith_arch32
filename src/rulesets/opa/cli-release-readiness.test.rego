...
test_compliant_release_readiness_has_no_violations if {
	violations := cli_release_readiness.violations with input as compliant_input
	count(violations) == 0
}

test_build_failure_is_rejected if {
...
test_mcp_smoke_failure_is_rejected if {
