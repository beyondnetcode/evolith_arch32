...
**Title:** Tres checks de CI en el PR #275 están en rojo con fallos preexistentes ya identificados — solo investigación, sin arreglo aplicado

- **Purpose:** Distinguir, con evidencia en vez de suposición, qué parte del CI en rojo de un PR es deuda preexistente de la rama frente a una regresión que introdujo el PR — y dejar un registro lo bastante preciso como para que arreglar cada uno sea una tarea acotada, no una reinvestigación.
- **Evidence:** **Tres checks de CI están en rojo en el PR #275 — `Security Audit`, `E2E Tests`, `Evolith Core Validation` — y los tres son deuda preexistente que este PR no introdujo, confirmado contra el `#273` ya mergeado de la misma rama, no asumido a partir de una corrida local en verde.** `Security Audit`: `npm audit --audit-level=high` reporta 31 vulnerabilidades (29 high, 2 moderate) en dependencias transitivas de las cadenas `jest-snapshot` y `fast-uri`; los commits de este PR no tocan ningún `package.json` ni lockfile. `E2E Tests`: exactamente uno de 132 tests falla — `CLI E2E Tests - Deep Architecture Analysis › validate with architecture analysis › should validate architecture with --arch flag`, que afirma `expect([0, 2]).toContain(result.exitCode)` (comentario propio del test: "GT-580: 0 pass, 2 blocked verdict") y recibe el código de salida `1` en su lugar, una salida de error no manejado donde el contrato del test solo anticipa pass-o-bloqueado. La propia corrida de CI del `#273` sobre el mismo rango de commits muestra el mismo test fallando con el mismo `1 failed, 131 passed, 132 total`. `Evolith Core Validation` falla en su paso `Bilingual Terminology Lint` (`node .harness/scripts/bilingual-terminology-lint.mjs`): 5 inconsistencias de terminología — `Database`→`Base de datos`, `Ports and Adapters`→`Puertos y Adaptadores`, `Portability`→`Portabilidad`, `Modular Monolith`→`Monolito Modular`, `Microservices`→`Microservicios` — repartidas en `reference/core/architecture/adrs/core/0124-credential-secret-management-standard.es.md`, `reference/core/architecture/demos/level-3-components/README.es.md`, `reference/core/control-center/gaps/gap-tracking.es.md` (dentro de la prosa preexistente del catálogo de `GT-598`, no de ninguna fila que este PR añadió) y `reference/core/control-center/maturity-reports/maturity-assessment.es.md` (2 aciertos). `git log` sobre cada fichero confirma que ningún commit de este PR lo tocó.
- **Component:** `Cross` · **Criticality:** P2 · **Complexity:** S
- **Provenance:** Encontrado el 2026-07-29 al confirmar que el CI del PR #275 quedaba verde tras el arreglo del ratchet de `GT-634`. Investigado a pedido explícito del usuario de abrir un gap e investigar — no arreglar — así que este registro se detiene en una causa raíz confirmada por hallazgo, deliberadamente.
- **Acceptance criteria:**
  - [ ] `Security Audit`: los 31 hallazgos de `npm audit` quedan triados — actualizados donde exista un arreglo no disruptivo, o aceptados explícitamente con motivo documentado donde no lo haya (p. ej. una dependencia transitiva solo de desarrollo).
  - [ ] `E2E Tests`: la ruta del flag `--arch` que devuelve el código de salida `1` en vez de `0`/`2` queda con causa raíz identificada (¿es una excepción no capturada en la ruta de análisis de arquitectura profunda, o un fixture que el proyecto fixture no satisface?) y se corrige el CLI o la expectativa del test.
  - [ ] `Evolith Core Validation`: los 5 términos marcados se corrigen in situ, o la sugerencia del lint se descarta con motivo documentado por término (algunos pueden ser intencionales, p. ej. un nombre de producto que se mantiene en inglés).
  - [ ] Los tres reverificados en verde en una corrida de CI posterior de esta misma rama, no solo reproducidos en local.
