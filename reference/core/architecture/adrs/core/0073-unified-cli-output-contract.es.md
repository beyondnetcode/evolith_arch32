...
3. **Contrato unificado: envelope envolviendo payloads tipados por schema (elegida).** El envelope del lado Tracker se vuelve el wrapper universal; el `GateEvidence` del lado Core se vuelve el primer payload `data` tipado, publicado como JSON Schema en `src/rulesets/schema/`.
...
2. **`GateEvidence`** es el payload `data` de la evaluación de gates: `{ gateId, phase, verdict: passed|failed|skipped, rulesetRef, rulesetVersion, violations: [{ ruleId, severity: error|warning, location, message }], evaluatedAt, evaluatedBy: human|agent|ci }`. Publicado como `src/rulesets/schema/gate-evidence.schema.json` (entregable de GT-02). Los 27 rulesets ya tienen el campo `version` que `rulesetVersion` requiere (verificado 2026-06-10).
...
- Rulesets: `src/rulesets/cli/core-parity.rules.json`, futuro `src/rulesets/schema/gate-evidence.schema.json`
