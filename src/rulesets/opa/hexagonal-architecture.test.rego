...
test_compliant_hexagonal_has_no_violations if {
	violations := hexagonal_architecture.violations with input as compliant_input
	count(violations) == 0
}

test_core_framework_imports_is_rejected if {
...
test_application_infrastructure_imports_is_rejected if {
...
test_infrastructure_not_implementing_ports_is_rejected if {
