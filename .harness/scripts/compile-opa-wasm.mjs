...
function collectManifest() {
  ...
  // GT-716 AC1: rule id → the `input.…` paths its policy reads. Built from the same
  // parse, under the same reachability rule as the ids: a policy `main.rego` does not
  // import contributes neither.
  const ruleInputPaths = new Map();
  const unreachable = [];
  for (const file of files) {
    ...
    for (const [id, paths] of ruleInputPathsFromAst(ast)) {
      ruleInputPaths.set(id, [...new Set([...(ruleInputPaths.get(id) ?? []), ...paths])].sort());
    }
  ...
  return { declaredRuleIds: [...ruleIds].sort(), ruleInputPaths };
}
...
async function compileWasm() {
  ...
  writeFileSync(
    ...
    + '# GT-716: and, per rule id, the `input` paths the policy deciding it reads, so an\n'
    + '# absent fact can be reported as "not evaluated" instead of as a verdict.\n'
    + 'package evolith.manifest\n\nimport rego.v1\n\n'
    + `declared_rule_ids := {${declaredRuleIds.map((id) => JSON.stringify(id)).join(', ')}}\n\n`
    + `rule_input_paths := {${[...ruleInputPaths.keys()].sort().map((id) => `${JSON.stringify(id)}: ${JSON.stringify(ruleInputPaths.get(id))}`).join(', ')}}\n`,
  );
  ...
  const entrypoints = [
    ...
    '-e', 'evolith/manifest/rule_input_paths',
  ];
