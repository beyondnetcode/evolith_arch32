...
> Estas tres alertas ROJAS se rastrean para acción en el único [Tablero de Seguimiento de Gaps](../../control-center/gaps/gap-tracking.es.md) como `GT-110` (Kong → Traefik/NGINX), `GT-111` (MassTransit v9) y `GT-112` (HashiCorp → OpenTofu/OpenBao). Este audit sigue siendo la fuente de registro de vigilancia tecnológica.
