...
- **Procedencia:** Registrado el 2026-07-31 desde la corrida de CI `30631939687` y reproducido localmente el mismo día. Id asignado por unión de `origin/main..origin/develop` y los pull requests abiertos (`#325` y el conjunto de dependabot), cuyo máximo era `GT-645`. Separado de [`GT-647`](./gap-reference-catalog.es.md#gt-647) deliberadamente: los WARN de ese log son un *síntoma* de esta carga por petición pero son defectos de corpus independientes, y juntarlos es justo el fallo de un-gap-un-reclamo que [`GT-639`](./gap-reference-catalog.es.md#gt-639) existe para evitar.
- **Criterios de aceptación:**
  ...
  - [x] **Los veredictos no cambian, verificado en vez de argumentado.** Se capturó el `EvaluationResult` completo del payload de smoke de k6 desde una build de `origin/develop` y desde este cambio con los datos de fase de [`GT-647`](./gap-reference-catalog.es.md#gt-647) retenidos, y los dos son **idénticos byte a byte, 442 204 bytes**. Aparte, el propio corpus normalizado se comparó contra el loader previo al cambio: **393 reglas, JSON idéntico**.
...
**Título:** Tres documentos del corpus se reportan rotos en cada carga, la etapa de gates de fase no evalúa nada desde el 2026-07-04, y cada línea de log de un argumento se imprime dos veces

- **Propósito:** Hacer que el corpus de evaluación cargue limpio y completo — cada documento verificado contra el contrato que realmente declara, cada gate escrito alcanzable, y una línea de log por llamada de log.
- **Evidencia:** Tres fallos afloraron juntos en el `core-api.log` de la corrida de CI `30631939687`; cada uno es real por separado, y el segundo es el grave. **(1) TRES FICHEROS `*.rules.json` FALLAN EL SCHEMA ESTÁNDAR DE RULESET Y SE SALTAN EN CADA CARGA**, con `must have required property 'rules' / 'principles'`: `architecture/topology-recommendation.rules.json`, `infrastructure/helm-enforcement.rules.json`, `infrastructure/opa-sidecar-bundle.rules.json`. Ninguno tiene nada malo — son *tipos de documento distintos* que comparten el sufijo `*.rules.json`. El primero es el catálogo consultivo de recomendación de ADR-0104, que tiene su propio `topology-recommendation.schema.json` y su propio consumidor (`TopologyRecommendationService`, `topology recommend`); los otros dos son declaraciones de una sola regla con la forma de `rule-definition.schema.json`, aplicadas por el guard `29-validate-opa-sidecar-bundles.mjs` y sus políticas Rego pareadas, no por el motor de reglas. El loader no tenía vocabulario para "no es un ruleset del corpus", así que verificaba cada uno contra un schema que nunca debieron satisfacer y reportaba el resultado como rotura. **Peor, y sin notarse hasta esta fila: los tres también fallaban el schema que ellos mismos DECLARAN**, porque nada los validó nunca contra él — `severity: "error"` no está en el enum de ese schema, el campo es `title` y los ficheros escribían `name`, el bloque `validation` que los ficheros llevan desde el principio nunca se modeló, y las rutas `$schema` quedaron obsoletas con la reorganización de `reference/` (`../../schema/…` resuelve a un directorio inexistente). Sus globs `validation.target` apuntan a `reference/infrastructure/…`, un árbol que `f0d01911` eliminó. **(2) `reference/governance/sdlc/phases/` NO EXISTE, Y NO EXISTE DESDE EL 2026-07-04.** Los cinco ficheros `phase-f*.json` los borró `f0d01911` — *"chore(taxonomy): remove leftover old directories under reference/"* — que eliminó todo el árbol `reference/governance/`; `GT-461` (`061eacbd`) recreó después `gates/` **en esa misma ruta** y no recreó `phases/`. Esto no es un warning cosmético: `loadGatesForPhase` resuelve los gates *a través* del índice de fases, así que `loadAllPhases()` devolviendo `[]` deja **los cinco ficheros de gate escritos inalcanzables**. Durante unas cuatro semanas la etapa de gates de fase de `SatelliteEvaluationPipeline` produjo cero resultados de gate sin reportar error — una evaluación gobernada se saltaba en silencio la mitad del pipeline donde viven los gates del SDLC, y el veredicto de nivel superior se calculaba sin ellos. Confirmado contra un Core en ejecución: con los datos restaurados, `POST /api/v1/evaluate` devuelve `gate-f1` junto a `general-rulesets` donde `origin/develop` devuelve solo el segundo. **(3) `NestLoggerProvider` REENVÍA SU `context` OPCIONAL SIN CONDICIÓN.** El `Logger` de Nest lee `...optionalParams` por posición, así que `this.logger.warn(message, undefined)` no es la misma llamada que `this.logger.warn(message)` — el `undefined` explícito se vuelve una segunda cosa que imprimir. Cada `warn`/`debug`/`info`/`error` de un solo argumento del dominio emitía por tanto una línea `WARN undefined` pareada: **44 de ellas en 11 peticiones** en la reproducción base, duplicando el volumen de todo lo que el Core escribe y poniendo una línea sin sentido junto a cada línea real.
- **Componente:** `Core Domain` · **Criticidad:** P1 · **Complejidad:** S
- **Procedencia:** Registrado el 2026-07-31 desde el mismo `core-api.log` que produjo [`GT-646`](./gap-reference-catalog.es.md#gt-646), y mantenido aparte a propósito: esos WARN son cómo se *notó* la carga por petición, pero son defectos de corpus y de logging que sobrevivirían a cualquier cantidad de caché. El fallo (2) se rastreó con `git log --diff-filter=AD` en vez de suponerse — los ficheros existían, y el commit que los quitó dice que estaba limpiando sobras.
- **Criterios de aceptación:**
