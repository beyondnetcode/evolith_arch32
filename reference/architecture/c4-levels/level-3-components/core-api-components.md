...
The **Core API** is the stateless evaluation surface of Evolith. It receives HTTP REST requests, resolves workspace references to a safe filesystem location, evaluates canonical `EvaluationContext` payloads through `@evolith/core-domain`, serves reference/ruleset reads, and returns technical evaluation results.
...
C4Component
    ...
    Container_Boundary(api, "Core API Container") {
        ...
        Component(orchestrator, "EvaluationOrchestrator", "@evolith/core-domain/evaluation", "Canonical stateless evaluation entry point for EvaluationContext and EvaluationResult.")
        Component(usecases, "Use Cases", "@evolith/core-domain/application", "Orchestrates validation, gate evaluation, phase transition proposals, satellite validation, and architecture checks.")
...
| **@evolith/core-domain** | Core business logic and contracts, decoupled from NestJS. Defines evaluation contexts/results, gates, evidence, phase transitions, events, providers, and validators. |
