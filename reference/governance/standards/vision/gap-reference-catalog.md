...
- **Residual risk:** stages 3 (public SDLC enums, outward-facing — keep `f1..f5` as deprecated alias), 4b (retire F# maturity VALUES → canonical), 2b (validate-workflow), 5 (OPA anti-collision) pending.
- **Done when:** [x] canonical PhaseId single source + alias normalizer (stage 1); [x] core-domain validators/services use it (stage 2 — 4/5 sites; validate-workflow = 2b); [ ] contract surfaces migrated, `f1..f5` accepted only as deprecated alias (stage 3); [x] topology `phase`→`maturityLevel` (stage 4); [ ] no namespace collision (stage 5 OPA guard); [ ] all suites green.
