...
| Resilience | 4/10 | The Core is a stateless evaluation engine (ADR-0101) that makes no outbound DB or HTTP calls, so it has almost nothing to protect. The previously-scored `opossum` circuit breaker was a DI-registered orphan (zero injections, zero `createBreaker` callers) and has been REMOVED, together with the dead Redis cache wiring that never connected — caching is now honestly per-process/in-memory (GT-560, closed 2026-07-24). The 4/10 reflects that minimal resilience infrastructure is the correct shape for a service that calls nobody, not a missing capability; there is no longer an unsubstantiated claim inflating it. If a shared cache is ever introduced it must be added jointly with a breaker (an unreachable Redis hangs `get`/`set` rather than rejecting) |
