...
- **e2e suite (`test:e2e`) — partial:** fixed the TS-rot e2e specs (sdlc-gate-commands-e2e + wizard.e2e now green; gate.e2e-spec address cast). e2e went 5→3 failing suites (151 passing). **3 integration suites remain (pre-existing, env-dependent, NOT unit-spec rot, NOT regressions — unit suite is 905/905 green):** cli-e2e (`validate` is the default/root command; the e2e invokes it as a subcommand), mcp-e2e (spawns an MCP HTTP server — ports/session), gate.e2e-spec (gate evaluation needs artifact fixtures like docs/prd.md). These belong with the e2e cross-surface coverage work (cf. GT-223), not the GT-345 unit rot.
- **Done when:** [x] unit suite green (GT-345 core); [x] e2e TS-rot fixed; [ ] the 3 integration e2e suites green so full `npm test` passes (env setup — separate workstream).
