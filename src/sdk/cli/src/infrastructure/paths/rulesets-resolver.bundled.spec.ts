 ...
 *
 * These three cases originally read the real bundled tree at
 * `src/sdk/cli/rulesets/`. That tree is a BUILD ARTIFACT: it is gitignored
 * (`rulesets/*`, minus the committed `rulesets/agents/`) and generated by
 * `scripts/copy-rulesets.js` on the CLI's `prebuild`. CI installs with `npm ci`
 * at the workspace root, which never triggers that prebuild, so in CI only
 * `rulesets/agents/` exists — and `agents` is deliberately NOT a corpus marker,
 * so resolution correctly failed and these tests failed with it. They passed
 * locally only on trees carrying a copy from an earlier build.
 *
