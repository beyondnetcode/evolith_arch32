...
import { REPO_ROOT, RULESETS_ROOT, triageCorpus } from '../../../test/rule-corpus-triage';
 ...
 * rather than merely printed. Measured 2026-07-29 against 386 rules.
 *
 * `native-handler` went 139 -> 151 on 2026-07-29, from TWO independent closures
 * that landed together and touch DISJOINT rule sets, so their effects add:
 *   +8  the config-shaped rules (GT-595) — ED-R04/R05/R06 and DAM-R05 (topology
 *       flags, claimed by ID because their bare categories are shared across
 *       topologies) and MTN-05, GIT-08, SEC-RL-01, SEC-RL-02 (config assertions
 *       in GovernanceRuleHandler);
 ...
 *       `enforce` was dropped at normalization. It is now carried — in
 *       `test/rule-corpus-triage.ts` as well as in DiskRulesetRepository, or the
 *       measurement would once again be taken over a different corpus than the
 *       one production evaluates — and `ModuleBoundaryRuleHandler` evaluates it.
