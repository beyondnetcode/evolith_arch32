...
- **Riesgo residual:** etapas 3+ pendientes — superficies de contrato (enum schema config, DTO composable-validate, `--phase` CLI, `phase`→`maturityLevel` en manifest) + validate-workflow (2b) + regla OPA anti-colisión. La etapa 3 es outward-facing (Tracker) — política por defecto: mantener `f1..f5` como alias aceptado-pero-deprecado, no eliminación dura.
- **Hecho cuando:** [x] fuente única PhaseId + normalizador (etapa 1); [x] validators/services lo usan (etapa 2 — 4/5 sitios; validate-workflow = 2b); [ ] superficies de contrato migradas; [ ] `phase`→`maturityLevel` en topología; [ ] sin colisión; [ ] suites verdes.
