...
violations contains {"id": "CB-VAL-01", "message": msg} if {
	spec_declared
...
violations contains {"id": "CB-VAL-02", "message": msg} if {
	spec_declared
...
violations contains {"id": "CB-01", "message": "Technology selection must be validated against Agnostic Baseline. Declare spec.compliance.agnosticBaseline in evolith.yaml pointing to the authoritative tech stack document."} if {
	spec_declared
...
violations contains {"id": "CB-02", "message": "Product architecture must be traceable to the Reference Blueprint. Declare spec.compliance.referenceBlueprint in evolith.yaml."} if {
	spec_declared
...
violations contains {"id": "CB-03", "message": "Engineering Manifesto principles must be enforced. Declare spec.compliance.engineeringManifesto in evolith.yaml."} if {
	spec_declared
	not spec.compliance.engineeringManifesto
}

violations contains {"id": "CB-03", "message": "Engineering Manifesto requires linting enforcement. No CI workflow containing 'lint' detected in satellite .github/workflows/."} if {
	spec_declared
...
violations contains {"id": "CB-04", "message": "Definition of Done must be satisfied before work-item closure. Declare spec.compliance.definitionOfDone in evolith.yaml."} if {
	spec_declared
...
violations contains {"id": "CB-05", "message": "Repository structure must follow Taxonomy rules. Declare spec.compliance.repositoryTaxonomy in evolith.yaml."} if {
	spec_declared
	not spec.compliance.repositoryTaxonomy
}

violations contains {"id": "CB-05", "message": "Repository Taxonomy requires 'src' directory. Satellite is missing expected top-level directory."} if {
	spec_declared
