...
- **Suite e2e (`test:e2e`) — parcial:** corregidos los specs e2e con TS-rot (sdlc-gate-commands-e2e + wizard.e2e verdes; cast en gate.e2e-spec). e2e pasó de 5→3 suites en rojo (151 pasando). **Restan 3 suites de integración (preexistentes, dependientes del entorno, NO unit-rot, NO regresiones — unit 905/905):** cli-e2e (`validate` es el comando raíz/default; el e2e lo invoca como subcomando), mcp-e2e (levanta servidor MCP HTTP — puertos/sesión), gate.e2e-spec (la evaluación de gate necesita fixtures de artefactos como docs/prd.md). Pertenecen al trabajo de cobertura e2e cross-surface (cf. GT-223).
- **Hecho cuando:** [x] suite unitaria verde (núcleo GT-345); [x] TS-rot de e2e corregido; [ ] las 3 suites e2e de integración verdes para que `npm test` completo pase (setup de entorno — workstream aparte).
