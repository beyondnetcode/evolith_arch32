...
- **Residual risk:** stages 3+ pending — the contract surfaces (config schema enum, composable-validate DTO, CLI `--phase`, topology manifest `phase`→`maturityLevel`) + validate-workflow (2b) + OPA anti-collision rule. Stage 3 is outward-facing (Tracker) — default policy: keep `f1..f5` as accepted-but-deprecated alias, not a hard removal.
- **Done when:** [x] canonical PhaseId single source + alias normalizer (stage 1); [x] core-domain validators/services use it (stage 2 — 4/5 sites; validate-workflow = 2b); [ ] contract surfaces migrated, `f1..f5` accepted only as deprecated alias; [ ] topology `phase`→`maturityLevel`; [ ] no namespace collision; [ ] all suites green.
