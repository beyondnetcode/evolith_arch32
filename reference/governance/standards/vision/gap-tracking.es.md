...
| [`GT-382`](./gap-reference-catalog.es.md#gt-382) | Continuación R4 — los veredictos OPA context-aware son inertes: `OpaEvaluator` filtra violaciones por `v.id === rule.id` (id derivado del path `opa-dod`), descartando los facts threadeados `DOD-*`/`CB-*`/`PG-*`. Hacer que los facts de `input.context` afecten el veredicto sin regresión del path FS (guard sin-facts→sin-opinión). | `OPA` | Cross | P1 | M | `PENDIENTE` |
