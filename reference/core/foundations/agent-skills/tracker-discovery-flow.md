...
Capture the owner-guided decisions on **Evolith Tracker's Discovery phase**: its artifacts and criteria, the new **governed architecture-advisory** capability (first Tracker→Core-architecture bridge), the progressive blueprint, and the PRD model. Five decisions (D-001…D-005) closed the Discovery block on 2026-07-04.
...
| D-004 | **PRD mandatory** in Discovery. Only the PRD is **non-overrideable** (canonical floor, L-010); Canvas/BusinessCase/advisory are tenant-overrideable. | PRD is the non-negotiable Discovery artifact. | PRD schema in Core; Gate 1 always requires the PRD. |
...
- **PRD as canonical floor (D-004):** the PRD schema is a Core-corpus candidate (`src/rulesets/schema/`), inherited by Tracker and satellites; non-overrideable per L-010.
...
- Confirm which of Canvas / advisory are enabled by default vs. purely opt-in per tenant.
