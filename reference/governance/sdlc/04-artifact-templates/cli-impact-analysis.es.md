...
- `smart-cli sdlc init business-case` -> Genera `business-case-roi-template.es.md` y `business-case.yaml`.
- `smart-cli sdlc init ballpark` -> Genera `ballpark-estimation-template.es.md` y `ballpark.json`.
- `smart-cli sdlc init story` -> Genera `evolith-user-story-template.es.md`.
- `smart-cli sdlc init backlog` -> Genera `agile-backlog-template.es.md`.
...
- **Linter de Lenguaje Ubicuo:** `smart-cli sdlc check ubiquitous-language` parsea la Sección 1 del modelo DDD y cruza con los JSONs de User Stories para detectar desviaciones léxicas.
...
- `smart-cli sdlc handoff to-business-case --from discovery.json` -> El CLI lee el problema y el valor esperado y pre-puebla el YAML del Business Case.
- `smart-cli sdlc handoff to-ballpark --from business-case.yaml` -> El CLI extrae el ID de la iniciativa y el `targetTimeInMonths` para fijar límites en la estimación.
