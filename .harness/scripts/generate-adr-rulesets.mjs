...
function buildRuleset(adr) {
  ...
  const ruleset = {
    ...
    version: '1.0.0',
    // GT-571: every rule in a generated ADR ruleset asserts, among other things,
    // that the decision record it cites EXISTS on disk under
    // `reference/core/architecture/adrs/<track>/`. Only the Core repository has
    // that tree, so in a satellite the rule reports "the ruleset cites 1 decision
    // record(s) that do not exist" and blocks — on a file the satellite was never
    // supposed to have. Measured against a freshly initialised satellite on
    // 2026-07-28: all 91 blocking findings came from these generated rulesets and
    // NONE from a hand-authored rule, so the repository had nothing wrong with it.
    //
    // `core` is derived from what the rules check, not from a judgement about
    // whether satellites should honour Core decisions — they do, through the
    // inheritance model and the hand-authored rulesets that encode it. A
    // file-existence check against the Core's own ADR tree is a Core-repository
    // concern by construction, and it is emitted for every track for the same
    // reason: `nodejs`, `dotnet`, `ai-augmented` and `android` records live under
    // that same tree.
    audience: 'core',
    adrId: adr.adrId,
    adrTitle: adr.title,
