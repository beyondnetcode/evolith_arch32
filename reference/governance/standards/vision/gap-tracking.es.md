...
| [`GT-380`](./gap-reference-catalog.es.md#gt-380) | R4 — OPA `input.context` alineado a `EvaluationContext` (re-anclar `dod.rego` fuera de `input.story.*`; quitar artefactos de historia de mandatoryEvidence en `phase-gates`; multi-tenancy MTN-09..11 + ABAC scoping; paridad Native+OPA 0 drift) | `OPA` | Cross | P1 | M | `EN-PROGRESO` |
