...
test_rejected_missing_technical_scope if {
	sdlc_mode == "rejected" with input as {"scope": {"functional": "Add login"}}
}

test_rejected_missing_functional_scope if {
	sdlc_mode == "rejected" with input as {"scope": {"technical": "Add DB table"}}
}

test_full_mode_high_criticality if {
...
test_full_mode_with_security_risks if {
	sdlc_mode == "full" with input as {
		"scope": {
			"functional": "Add login",
			"technical": "Add DB table",
...
test_tailored_mode_medium_complexity if {
...
test_minimal_mode_low_complexity if {
