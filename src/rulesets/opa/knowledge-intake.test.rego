...
test_candidate_with_provenance_has_no_violations if {
	violations := knowledge_intake.violations with input as valid_candidate
	count(violations) == 0
}

test_missing_rights_is_rejected if {
...
test_executable_without_fixtures_is_rejected if {
...
test_missing_maturity_is_rejected if {
...
test_missing_source_registry_link_is_rejected if {
...
test_missing_review_freshness_is_rejected if {
...
test_accepted_without_adr_is_rejected if {
...
test_retired_without_disposition_is_rejected if {
...
test_evaluated_without_promoted_at_is_rejected if {
...
test_emergent_candidate_has_no_violations if {
	violations := knowledge_intake.violations with input as valid_emergent
	count(violations) == 0
}

test_emergent_without_repository_is_rejected if {
...
test_emergent_without_sensitivity_is_rejected if {
...
test_restricted_emergent_cannot_leave_candidate if {
...
test_promoted_emergent_requires_evidence_ref if {
	candidate := {"knowledge_id": "KO-TEST-001", "origin": {"class": "violation", "repository": "satellite-ums", "detected_by": "@winston-architect", "detected_at": "2026-07-18", "sensitivity": "internal"}, "assessment": {"maturity": "proven", "preconditions": ["domain-modeling"], "anti_patterns": ["anemic"], "alternatives": ["event-sourcing"]}, "review": {"owner": "@winston", "next_review_at": "2026-12-20", "review_freshness": "2026-07-18"}, "promotion": {"status": "evaluated", "promoted_at": "2026-07-18", "promoted_by": "@winston", "fixtures": []}}
	knowledge_intake.violations[_].id == "KO-R03" with input as candidate
}

test_record_without_axis_prefix_is_rejected if {
