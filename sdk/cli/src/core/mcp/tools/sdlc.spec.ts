import { handleSdlcTools } from './sdlc';
...
describe('MCP Tools - sdlc', () => {
  ...
  describe('evolith-sdlc-status', () => {
    ...
    it('should return phase-0 when evolith.yaml not found', async () => {
      ...
      const result = await handleSdlcTools('evolith-sdlc-status', { path: '/test/repo' });
  ...
  describe('evolith-sdlc-handoff', () => {
    it('should return error when path is missing', async () => {
      const result = await handleSdlcTools('evolith-sdlc-handoff', { fromPhase: 'phase-0', toPhase: 'phase-1' });
    ...
    it('should throw error when fromPhase is missing', async () => {
      await expect(handleSdlcTools('evolith-sdlc-handoff', { path: '/test/repo', toPhase: 'phase-1' }))
        .rejects.toThrow('Invalid phase');
    });

    it('should throw error when toPhase is missing', async () => {
      await expect(handleSdlcTools('evolith-sdlc-handoff', { path: '/test/repo', fromPhase: 'phase-0' }))
        .rejects.toThrow('Invalid phase');
    });

    it('should throw error for invalid phase transition', async () => {
      await expect(handleSdlcTools('evolith-sdlc-handoff', {
        path: '/test/repo',
        fromPhase: 'phase-2',
        toPhase: 'phase-0',
      }))
  ...
  describe('unknown tool', () => {
    it('should throw error for unknown SDLC tool', async () => {
      await expect(handleSdlcTools('evolith-sdlc-unknown', { path: '/test/repo' }))
        .rejects.toThrow('Unknown SDLC tool');
    });
  });
});
