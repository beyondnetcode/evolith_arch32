...
Satellites (external projects) submit evidence to Core via REST, MCP, or CLI. Core loads the SDLC gate definitions (`reference/core/sdlc/gates/gate-f*.json`), resolves the satellite's artifact paths, runs OPA/native rules, and emits a `Verdict` (PASS/FAIL/WAIVE/SKIP). All decisions are emitted as domain events, written to the audit ledger, and dispatched to webhook subscribers.
...
- [ ] Read `reference/core/sdlc/governance/core/README.md` — architecture decisions
- [ ] Read `CERTIFICACION_MADUREZ.md` — current maturity certification
- [ ] Read `reference/core/control-center/gaps/gap-tracking.md` — gap board
...
- [ ] Read `reference/core/sdlc/README.md` — SDLC model overview
...
1. Pick a gap from `reference/core/control-center/gaps/gap-tracking.md` marked `PENDING`.
