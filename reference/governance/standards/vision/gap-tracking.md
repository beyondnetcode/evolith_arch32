...
| [`GT-382`](./gap-reference-catalog.md#gt-382) | R4 follow-on — context-aware OPA verdicts are inert: `OpaEvaluator` filters violations by `v.id === rule.id` (path-derived `opa-dod`), discarding threaded `DOD-*`/`CB-*`/`PG-*` facts. Make threaded `input.context` facts affect the gate verdict without an FS-path regression (add no-facts→no-opinion guard). | `OPA` | Cross | P1 | M | `PENDING` |
