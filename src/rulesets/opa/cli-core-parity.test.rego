...
test_compliant_core_parity_has_no_violations if {
	violations := cli_core_parity.violations with input as compliant_input
	count(violations) == 0
}

test_rule_without_parity_record_is_rejected if {
...
test_divergent_logic_is_rejected if {
...
test_inconsistent_results_is_rejected if {
...
test_undocumented_parity_gap_is_rejected if {
