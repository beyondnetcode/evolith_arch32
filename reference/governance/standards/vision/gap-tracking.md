...
| [`GT-386`](./gap-reference-catalog.md#gt-386) | R3 — Durable persistence adapters: replace `InMemorySchedulerAdapter` (no timers; lost on restart) with a durable cron/queue adapter behind `ISchedulerPort`, and `InMemoryMemoryAdapter` with a persistent store behind `IMemoryPort`. In-memory stays the test default. | `Agent Runtime` | Cross | P2 | M | `PENDING` |
...
**Progress:** 378 / 389 done · 6 in progress · 5 pending · 0 deferred
