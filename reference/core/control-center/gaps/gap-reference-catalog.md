...
- **Acceptance criteria:**
  - [ ] `gh pr checks` on a fresh PR **into `develop`** shows no `CodeQL` check reporting "configuration not found" — `develop`, not `main`, because that is the ref the warning names and the only base still carrying it. Open pending observation on the PR that lands this closure, which targets `develop` for exactly that reason.
