...
**Scope:** Winston agentic review and RAG index preparation in GitHub Actions.

## Purpose

Operate the Gemini-backed Winston review and the RAG chunking pipeline safely. This guide covers support procedures; it does not define the architectural rules reviewed by Winston.
...
The `Winston Agentic Review` CI job supplies the secret with `EVOLITH_AGENTIC_REVIEW=true`. The job fails when the secret is absent, Gemini cannot be contacted, or Winston reports a violation. Never place the key in source code, repository variables, logs, issues, or a committed `.env` file. Rotate a key immediately after exposure.
...
| `VIOLATION_DETECTED` | Winston found a governed architecture issue | Treat it as a blocking review finding; correct code or documentation and rerun CI. |
...
2. Review the `Winston Agentic Review` job log for the actual Gemini result.
...
- [Winston Audit Playbook](../../.harness/playbooks/wilson-audit-playbook.md)
