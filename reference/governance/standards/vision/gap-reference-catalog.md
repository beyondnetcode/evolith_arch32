...
### Component Tracker — Consolidated from the Tracker Audit

> These items were merged from the Evolith Tracker gaps-and-opportunities audit (`docs/audit/tracker-gaps-opportunities-tracking.md`, repo `evolith_tracker`) into this single formal tracking center, typified by the `Tracker` component. Their 93 already-resolved findings (COH/GAP/OPP series) remain archived in that audit document; only open or deferred items are carried here as `GT-xx`.

#### GT-83

**Title:** Unblock Tracker upstream dependencies

- **Gap:** Three upstream capabilities the Tracker depends on are not yet available: the Core API contract, UMS JWKS, and the UMS Auth Graph. They block Phase 1 implementation (originally `GAP-004`, CRITICAL/BLOCKED).
- **Purpose:** Resolve or formally contract the upstream dependencies so the Tracker's Phase 1 is unblocked.
- **Done when:**
  - [ ] each upstream dependency is delivered or stubbed against an agreed contract
  - [ ] the Tracker Phase 1 stories that depend on them are unblocked
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`GAP-004`) · [GT-42](#gt-42)

#### GT-84

**Title:** Extract AuditTrail as a shared kernel

- **Gap:** Five-plus bounded contexts implement immutable audit history independently; the duplication should be lifted into a shared kernel (originally `OPP-002`, deferred to Phase 8 / hardening).
- **Purpose:** Remove duplicated immutable-audit logic by extracting a single shared-kernel AuditTrail consumed by every context.
- **Done when:**
  - [ ] a shared-kernel AuditTrail abstraction exists and the duplicated implementations are migrated to it
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-002`)

#### GT-85

**Title:** Plugin architecture for the Tracker workflow engine
...
**Title:** Event-sourced aggregate roots

- **Gap:** Key aggregates (Project, Sprint, Release) persist current state only, losing temporal history and replay (originally `OPP-010`).
- **Purpose:** Adopt event sourcing for key aggregates to enable temporal queries, audit trails, and "what-if" replay.
- **Done when:**
  - [ ] selected aggregates persist an event stream and rebuild state from it
  - [ ] temporal queries and replay are demonstrated
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-010`)

#### GT-87

**Title:** GraphQL federation over REST

- **Gap:** Clients over-fetch from REST; dynamic dashboards need to query exactly what they require across services (originally `OPP-011`).
- **Purpose:** Expose a federated GraphQL schema via the BFF, with downstream services publishing subgraphs.
- **Done when:**
  - [ ] the BFF exposes a federated GraphQL schema
  - [ ] at least one downstream subgraph is composed and queried
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-011`)

#### GT-88

**Title:** Feature flags as first-class domain objects

- **Gap:** Feature flags live in configuration rather than the domain, so workflow transitions cannot gate on flag state (originally `OPP-012`).
- **Purpose:** Model flags in the domain and attach them to `RequirementChecklist` so workflow transitions gate on flag state.
- **Done when:**
  - [ ] flags are domain objects with lifecycle and audit
  - [ ] a workflow transition gates on flag state
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-012`)

#### GT-89

**Title:** Dynamic form/schema engine

- **Gap:** Custom fields, workflow forms, and reports are not data-driven (originally `OPP-013`).
- **Purpose:** Power custom fields/forms/reports via JSON Schema + UI Schema, with per-tenant schemas stored in `@evolith/tenant-config`.
- **Done when:**
  - [ ] forms render from per-tenant JSON Schema + UI Schema
  - [ ] schemas are versioned per tenant
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-013`)

#### GT-90

**Title:** CQRS with read-model projections

- **Gap:** Dashboards and reports read directly from write aggregates, coupling read and write concerns (originally `OPP-014`).
- **Purpose:** Separate write models from read-model projections that can be rebuilt on schema change without downtime.
- **Done when:**
  - [ ] read models are projected from domain events
  - [ ] a projection can be rebuilt without downtime
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-014`)

#### GT-91

**Title:** Contract testing with Pact (BFF↔Core)

- **Gap:** The satellite→upstream boundary (BFF↔Core) has no enforced contract tests, so breaking changes can slip through (originally `OPP-015`).
- **Purpose:** Enforce BFF↔Core contracts with Pact, run in CI on every PR.
- **Done when:**
  - [ ] Pact consumer/provider tests cover the BFF↔Core boundary
  - [ ] the contract verification runs in CI and blocks on break
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-015`) · [GT-42](#gt-42)

#### GT-92

**Title:** ADRs as code with a compliance CLI

- **Gap:** Architecture decisions are not machine-checkable; compliance (e.g. "no direct Core calls") is not enforced (originally `OPP-016`).
- **Purpose:** Store ADRs in `docs/adr/` with a CLI that validates compliance and auto-generates the `DECISIONS.md` table.
- **Done when:**
  - [ ] a CLI validates at least one ADR compliance rule
  - [ ] `DECISIONS.md` is generated from the ADR set
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-016`)

#### GT-93

**Title:** Built-in observability

- **Gap:** Traces do not span BFF→Core→integrations, and workflow transitions are not correlated with business metrics (originally `OPP-017`).
- **Purpose:** Make OpenTelemetry traces span the full path and correlate workflow transitions with cycle time and lead time.
- **Done when:**
  - [ ] a single trace spans BFF→Core→integration
  - [ ] workflow transitions are correlated with business metrics
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-017`)

#### GT-94

**Title:** Workflow DSL / visual editor

- **Gap:** Product owners cannot define SDLC workflows without engineering; there is no version-controlled, testable definition format (originally `OPP-018`).
- **Purpose:** Let product owners define SDLC workflows visually (BPMN-like), compiled to the engine's rule format, version-controlled and testable.
- **Done when:**
  - [ ] a workflow definition authored visually compiles to the engine format
  - [ ] the definition is version-controlled and testable
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-018`) · [GT-85](#gt-85)

#### GT-95

**Title:** Multi-tenant isolation with shared kernel

- **Gap:** Tenant isolation strategy for shared-core vs tenant extensions is not formalized (originally `OPP-019`).
- **Purpose:** Use PostgreSQL row-level security plus schema-per-tenant for extensions, keeping the core schema shared and tenant extensions isolated.
- **Done when:**
  - [ ] RLS enforces tenant isolation on the shared core schema
  - [ ] tenant extensions are isolated per schema
- **References:** Evolith Tracker audit `docs/audit/tracker-gaps-opportunities-tracking.md` (`OPP-019`)

#### GT-96

**Title:** Real-time sync via WebSockets/SSE

- **Gap:** Aggregate changes are not pushed to clients, blocking collaborative backlog grooming and sprint planning (originally `OPP-020`).
- **Purpose:** Push aggregate changes to clients instantly via WebSockets/SSE.
- **Done when:**
