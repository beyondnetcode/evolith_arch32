...
- **Agent Runtime API `/v1/agent`** — the HTTP surface for governed agent orchestration (`apps/agent-runtime-api`, NestJS, fail-closed API-key/JWT guard): `POST handle`, `POST converse`, `POST hermes`, `POST stream` (SSE), `GET skills`, plus version-neutral `/health` and `/metrics`.
