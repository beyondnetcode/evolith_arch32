...
test_compliant_multi_tenancy_has_no_violations if {
	violations := multi_tenancy.violations with input as compliant_input
	count(violations) == 0
}

test_missing_application_filtering_is_rejected if {
...
test_missing_database_enforcement_is_rejected if {
...
test_missing_tenant_context_propagation_is_rejected if {
...
test_missing_api_tenant_validation_is_rejected if {
