...
- **Riesgo residual:** etapas 4b (retirar VALORES F# → canónico), 2b (validate-workflow), 5 (OPA anti-colisión) pendientes.
- **Hecho cuando:** [x] fuente única PhaseId + normalizador (etapa 1); [x] validators/services lo usan (etapa 2 — 4/5 sitios; validate-workflow = 2b); [x] superficies de contrato migradas (etapa 3); [x] `phase`→`maturityLevel` en topología (etapa 4); [ ] sin colisión (etapa 5 OPA); [ ] suites verdes.
